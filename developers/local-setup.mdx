...
For production or staging deployments, Pawtograder ships a Helm chart that bundles the Next.js web app with all Supabase services (Postgres, GoTrue, PostgREST, Realtime, Storage, Edge Functions, Kong, Studio).

### Quick start

```bash
# 1. Generate the JWT bundle
npx tsx scripts/GenerateJwtKeys.ts --env > .secrets/pawtograder-jwt.env

# 2. Create secrets in your namespace
...
The repository includes a GitHub Actions workflow (`.github/workflows/preview.yml`) that automatically deploys a full preview environment for each pull request and tears it down when the PR closes. Previews are seeded with demo data and accessible at `pr-<number>.preview.pawtograder.net`.

### Chart documentation

See `charts/pawtograder/README.md` in the repository for full documentation including:

- Required secrets and their keys
- Staging overlay (`charts/pawtograder/examples/values-staging.yaml`)
- Preview overlay (`charts/pawtograder/examples/values-preview.yaml`)
- Monitoring (Prometheus ServiceMonitors + Grafana dashboards)
- Shared Redis for cross-replica cache and rate-limiter state
