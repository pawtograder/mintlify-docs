...
See the [MCP server guide](/developers/mcp-server) for details on the AI assistance endpoint exposed at `http://127.0.0.1:54321/mcp`.
...
### Deployment branding variables

Self-hosted deployments can re-brand the app without rebuilding the web image. These variables are read server-side at request time and passed to the client via React context, so the same published image can be re-skinned per deployment.

| Variable | Description |
|----------|-------------|
| `BRAND_NAME` | Product name shown in page titles, headings, and wordmarks (default: `Pawtograder`) |
| `BRAND_DESCRIPTION` | `<meta name="description">` content |
...
| `BRAND_LOGO_DARK` | Logo for dark mode — same format as `BRAND_LOGO_LIGHT` |
...
| `BRAND_SSO_PROVIDERS` | JSON array of SSO buttons for the sign-in page (see below) |

#### Custom logos and favicon

To bake custom logos or a favicon into the image, place files in `public/branding/` before running `docker build`. Everything under `public/` is served at `/branding/*`. Then point the branding variables at those paths:

```bash
BRAND_LOGO_LIGHT=/branding/logo-light.png
BRAND_LOGO_DARK=/branding/logo-dark.png
BRAND_FAVICON=/branding/favicon.png
```

Alternatively, point the variables at any absolute `https://` URL to use externally hosted assets.

#### Configuring SSO buttons

`BRAND_SSO_PROVIDERS` is a JSON array of sign-in button definitions. Each entry requires a `provider` (a Supabase/GoTrue OAuth provider id) and a `label`. The `icon` and `scopes` fields are optional.

```bash
BRAND_SSO_PROVIDERS='[{"provider":"azure","label":"Continue with Microsoft","icon":"microsoft","scopes":"email User.Read"}]'
```

Supported `icon` values: `microsoft`, `github`, `google`, `apple`, `discord`, `gitlab`, `slack`, `twitch`, `linkedin`, `sso`, `generic`.

Leave `BRAND_SSO_PROVIDERS` unset to keep the default Microsoft (Northeastern Login) button. Set it to `[]` for email-only sign-in.

<Note>
Each provider listed in `BRAND_SSO_PROVIDERS` must also be enabled in GoTrue. See the Helm chart README for full deployment configuration details.
</Note>
...
| Variable | Description |
|----------|-------------|
| `LTI_KEY_ENCRYPTION_SECRET` | **32-byte, base64-encoded** key for encrypting tool RSA signing keys at rest. Generate with `openssl rand -base64 32`. |
| `LTI_TOOL_ISSUER` | The tool's public base URL (e.g. `https://pawtograder.example.edu`). Used to build OIDC redirect URIs. |
| `LTI_STATE_SECRET` | Signs the OIDC `state` (CSRF) token. Falls back to `LTI_KEY_ENCRYPTION_SECRET` if unset. |
| `LTI_CRON_SHARED_SECRET` | Shared secret that authenticates scheduled roster-sync and grade-push calls from pg_cron. |
