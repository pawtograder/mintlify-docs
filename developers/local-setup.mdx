...
That prints `API_URL`, `SERVICE_ROLE_KEY` (labeled `Secret` in the CLI's interactive output), `ANON_KEY` (labeled `Publishable`), `DB_URL`, and the rest. Set these in `.env.local`:
