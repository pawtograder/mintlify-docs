...
| `hash` | Deterministic HMAC tokens derived from a caller-supplied `--salt`. Tokens are joinable across runs that use the same salt. |
| `raw` | Real student IDs, emails, and names. Requires `--i-understand-pii`. |

```bash
# Opaque (default) — safe for sharing, not joinable across runs
npm run cli -- assessment export --class cs3500-spring-2026

# Hash — joinable across runs with the same salt
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --identity hash \
  --salt "my-secret-salt-at-least-16-chars"

# Raw — real PII, requires explicit acknowledgement
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --identity raw \
  --i-understand-pii
...
| `--salt` | Salt for `--identity hash` (min 16 characters) |
| `--i-understand-pii` | Required for `--identity raw` |
| `--output`, `-o` | Output directory (default: `./assessment-export-<class>-<timestamp>`) |
| `--assignment`, `-a` | Assignment selector (ID, slug, or glob like `hw-*`). Repeatable. Omit to export all. |
...
When you used `--identity hash`, you can export a CSV mapping subject tokens back to real student identifiers:
...
<Warning>
The deanonymize output contains real student PII. The file is created with restrictive permissions (`0o600`). Handle it accordingly.
</Warning>
