...
If you have a checkout of the platform repository, you can also run the CLI directly:
...
You can also provide the token directly (useful for CI):
...
All commands follow the pattern `pawtograder <group> <action> [options]`. Add `--help` to any group or action for its full option list. All `list` commands accept `--json` to emit the raw JSON response for piping into `jq` or other tools.
...
pawtograder classes list --json | jq '.classes[] | .name'
```
...
Use `--strip-ids` to export a rubric as a template for a new assignment (every row is created fresh on import rather than updating existing rows).
...
Use `--dry-run` to validate the import against the live rubric and see exactly what would change (row additions, removals, point changes, grading-mode changes) without writing anything.
...
List discussion board topics with thread and question counts:
...
`reviews assign` distributes submissions across graders using a balanced round-robin algorithm. It honors grading conflicts and never assigns someone their own submission.
...
A bare `--due-date` (e.g. `2026-09-15`) means end of that day in the class's time zone. Append a time (`2026-09-15T17:00`) for a specific time.
...
Where `drafts.json` is an array of `{"assignee_profile_id": "...", "submission_id": 123, "rubric_part_id": 45}` objects.
...
<Note>
This command is not available for repo-only assignments (those with the autograder disabled). Use the assignment's **Autograder** page to toggle the autograder and sync the workflow change to student repositories instead.
</Note>
...
- `--json` – Emit the raw JSON response (all `list` commands)
...
```bash
# Preview the grading distribution
pawtograder reviews assign \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
...
If you see an HTML response error, you may have passed the web host URL instead of the API gateway origin. Use `pawtograder login --url https://api.your-school.edu` for self-hosted deployments.
...
If `pawtograder` is not found after installing globally, ensure your npm global bin directory is on your `PATH`. Alternatively, use `npx @pawtograder/cli <command>`.
