...
If you are working inside the platform repository, you can also run the CLI directly:
...
Every `list` command accepts `--json` to emit the raw JSON response instead of a formatted table, which is useful for scripting:
...
A bare date (`YYYY-MM-DD`) is interpreted as the end of that day in the class's time zone. Append a time (`T17:00`) for a specific time, or include an explicit offset to bypass the class zone.
...
Use `--dry-run` to validate the import and preview what would change without writing anything. Use `--verbose` to also print the parsed rubric tree.
...
List submissions for an assignment, including active submission, autograder score, and total score:
...
The `--status` option accepts `open`, `in_progress`, `resolved`, `closed`, `active` (open or in_progress), or `all` (default).
...
Options include `--status pending|completed|all`, `--assignee <name>`, and `--rubric grading|self_review|meta`.

#### Assign grading work

Create review assignments balanced round-robin across active graders and instructors. The allocator honors `grading_conflicts` and never assigns someone their own submission.
...
The manifest is a JSON array of `{"assignee_profile_id": "...", "submission_id": 1, "rubric_part_id": null}` objects. Explicit manifests are validated against the same grading-conflict and self-review exclusions as the round-robin.

A bare `--due-date` (e.g. `2026-09-15`) means the end of that day in the class's time zone. Append a time (`T17:00`) for a specific time.

Re-running `reviews assign` is safe: existing assignments are reused rather than duplicated.
...
<Note>
`repos sync-grade-workflow` is not available for repo-only assignments (autograder disabled). Use the **Autograder** page in the web UI to toggle the autograder, which syncs the workflow to all student repositories automatically.
</Note>
...
- `--dry-run` – Preview changes without making them
- `--json` – Emit raw JSON instead of a formatted table (all list commands)
...
| `DEBUG=1` | Same as `PAWTOGRADER_VERBOSE`, plus stack traces on unexpected errors. |
...
If the CLI returns an error like "API returned an HTML page", you are pointing at the web host instead of the API gateway. Re-run `pawtograder login --url https://api.<your-hostname>`.

### Command not found

If you installed globally, make sure your npm global bin directory is on your `PATH`. Alternatively, use `npx @pawtograder/cli <command>`.
