...
The identifier can be a class ID, slug, or name. Add `--json` to any list command for machine-readable output.
...
A bare date (`2026-09-15`) means end of that day in the class's time zone. Append a time (`T17:00`) for a specific time, or include an offset (`T17:00-05:00`) to bypass the class zone.
...
Add `--strip-ids` to export a rubric as a template for use in a different assignment (every row will be created new on import rather than updating in place).
...
Use `--dry-run` to validate the YAML against the live rubric and see exactly what would change (insertions, deletions, point changes, reparenting) before writing anything.
...
Add `--include-non-submitters` to also list enrolled students who have not submitted. Add `--json` for machine-readable output.
...
Filter by status (`--status pending|completed|all`) or assignee (`--assignee <name>`). Add `--json` for machine-readable output.

#### Assign grading work

Distribute review assignments across graders and instructors using a balanced round-robin algorithm. Honors grading conflicts and never assigns someone their own submission.
...
A bare `--due-date` means end of that day in the class's time zone. Append a time (`2026-09-15T17:00`) for something specific.
...
Filter by status (`--status open|in_progress|resolved|closed|active|all`) or queue (`--queue <name>`). Add `--json` for machine-readable output.
...
Options: `--status closed|resolved`, `--resolution-status self_solved|staff_helped|peer_helped|no_time|other`, `--notes "..."`.

### Discussions

List discussion board topics:
...
If you see an HTML page error, you may be pointing at the web host instead of the API gateway. Use `pawtograder login --url https://api.<hostname>`.
...
If using the npm package, ensure it is installed: `npm install -g @pawtograder/cli`. If running from the repository, run from the platform repository root with `npm install` completed.
