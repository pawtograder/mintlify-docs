...
```bash
# Default (opaque tokens, all assignments, all gradebook columns)
...
# Specific assignments and gradebook columns
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --assignment hw-01 --assignment hw-02 \
  --gradebook-column final-grade

# Include autograder test output (truncated to 4 KB per test by default)
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --with-test-output \
...
| `--salt` | Required for `--identity hash`; must be ≥ 16 characters |
| `--i-understand-pii` | Required for `--identity raw` |
| `--output` / `-o` | Output directory (default: `./assessment-export-<class>-<timestamp>`) |
| `--assignment` / `-a` | Assignment selector (id, slug, or glob like `hw-*`). Repeatable. Omit for all. |
| `--gradebook-column` | Gradebook column selector (id, slug, or glob). Repeatable. Omit for all. |
...
- `manifest.json` — metadata and grand totals
- `subjects.json` — enrolled students (tokenized or raw)
...
- `assignments/<slug>/` — per-assignment data (rubric, submissions, scores, tests, hints)
- `gradebook/scores.json` — instructor-side gradebook cells

#### Deanonymize (hash mode only)

After exporting with `--identity hash`, generate a CSV that maps subject tokens back to real student identifiers:
...
<Warning>
The deanonymize command writes real student names, emails, and SIS IDs to disk. Handle the output file accordingly.
</Warning>
