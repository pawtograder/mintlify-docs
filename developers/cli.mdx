...
Export a privacy-controlled snapshot of class assessment data — rubrics, final grading review scores, autograder test results, hint feedback, and gradebook columns — for downstream analysis.
...
| `hash` | Deterministic HMAC tokens derived from `--salt`; joinable across runs |
...
| `--identity` | Identity mode: `opaque` (default), `hash`, or `raw` |
| `--salt` | Salt for `--identity hash` (required; minimum 16 characters) |
| `--i-understand-pii` | Required for `--identity raw` — acknowledges that real student data will be written to disk |
...
The export writes a directory with the following structure:

```
assessment-export-<class>-<timestamp>/
  manifest.json          # Export metadata and totals
  subjects.json          # Enrolled students with section info
...
#### Deanonymize a hash-mode export

After running an export with `--identity hash`, you can produce a CSV that maps subject tokens back to real student identifiers:

```bash
npm run cli -- assessment deanonymize \
  --class cs3500-spring-2026 \
  --salt <same-salt-used-in-export> \
  --i-understand-pii \
  --output roster.csv
```
...
<Warning>
The `--salt` value must be identical to the one used in the original export run. Opaque-mode exports cannot be deanonymized because the salt is randomly generated per run and not stored.
</Warning>
