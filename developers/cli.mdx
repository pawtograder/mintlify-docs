...
- `assignments/<slug>/` — per-assignment subdirectory with `rubric.json`, `submissions.json`, `scores.json`, `tests.json`, `hints.json`, and `autograder.json`
...
| Mode | Description |
|------|-------------|
| `opaque` (default) | Random per-run salt; tokens are only joinable within a single export |
| `hash` | Deterministic HMAC tokens using a caller-supplied `--salt`; joinable across runs |
| `raw` | Real student IDs, emails, and names; requires `--i-understand-pii` |

```bash
# Opaque (default) — safe for sharing with analysts
npm run cli -- assessment export --class cs3500-spring-2026

# Hash — joinable across runs (use the same salt each time)
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --identity hash \
  --salt "my-secret-salt-at-least-16-chars"

# Raw — real PII; handle output carefully
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --identity raw \
  --i-understand-pii
...
| `--assignment`, `-a` | Export only specific assignments (repeatable; supports globs like `hw-*`) |
| `--gradebook-column` | Export only specific gradebook columns (repeatable; supports globs) |
...
| `--test-output-max-bytes` | Truncate test output at this byte limit (default 4096) |

#### Deanonymize an export

If you exported with `--identity hash`, you can produce a CSV mapping subject tokens back to real student identifiers:
...
<Warning>
The `--salt` value must be identical to the one used in the original export run. The deanonymize command requires `--i-understand-pii` because the output contains real student names and emails.
</Warning>
