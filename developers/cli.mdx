---
title: "CLI Interface"
description: "Command-line tools for managing courses, assignments, rubrics, and student repositories"
icon: "terminal"
---

# Pawtograder CLI (Beta)

The Pawtograder CLI provides command-line tools for instructors and site administrators to manage courses, copy assignments between semesters, import rubrics and submission data, and perform bulk repository operations.

<Warning>
The CLI is in beta. Commands and options may change in future releases.
</Warning>

## Installation

The CLI is published to npm as `@pawtograder/cli`. Run it without installing:

```bash
npx @pawtograder/cli --help
```

Or install globally to get the `pawtograder` command:

```bash
npm install -g @pawtograder/cli
pawtograder --help
```

Requires Node.js 20 or later.

You can also run it from the platform repository:

```bash
npm run cli -- <command> [options]
```

### Prerequisites

- Node.js 20 or later
- SSH access to GitHub (for repository operations)
- A Pawtograder API token with CLI access

## Authentication

Before using the CLI, authenticate with an API token.

### Creating an API token

1. Log in to the Pawtograder web app.
2. Navigate to **Settings → API Tokens**.
3. Create a new token with the `cli:read` scope for read commands, or `cli:write` for commands that change data.
4. Copy the token. It starts with `mcp_`.

### Logging in

```bash
pawtograder login
```

You'll be prompted to enter your API token. The token is stored in `~/.pawtograder/credentials.json`.

You can also provide the token directly (useful for scripts and CI):

```bash
pawtograder login --token mcp_your_token_here
```

For self-hosted deployments, point `--url` at your deployment's **API gateway origin** (not the web host):

```bash
pawtograder login --url https://api.your-school.edu
```

The web host serves the application; the CLI talks to the `cli` Edge Function behind the API origin. Passing the web host produces an HTML response and the CLI will tell you so.

### Checking authentication

```bash
pawtograder whoami
```

### Logging out

```bash
pawtograder logout
```

## Commands

### Classes

List all classes your token can see:

```bash
pawtograder classes list
pawtograder classes list --json   # machine-readable output
```

Show details for a specific class:

```bash
pawtograder classes show <class-id-or-slug>
```

### Assignments

List assignments for a class:

```bash
pawtograder assignments list --class <class-id-or-slug>
pawtograder assignments list --class <class-id-or-slug> --json
```

Show assignment details:

```bash
pawtograder assignments show <assignment-id-or-slug> --class <class-id-or-slug>
```

#### Copy assignments

Copy assignments between classes. Supports single-assignment copy, full-course copy, and schedule-driven copy:

```bash
# Copy a single assignment
pawtograder assignments copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --assignment hw1 \
  --workdir /tmp/pawtograder-repos

# Copy all assignments
pawtograder assignments copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --all \
  --workdir /tmp/pawtograder-repos

# Copy with a schedule CSV (custom due dates)
pawtograder assignments copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --schedule schedule.csv \
  --workdir /tmp/pawtograder-repos
```

The copy command handles:

- Assignment record creation
- Rubrics (deep copy)
- Autograder configuration
- Self-review settings
- Handout and solution repository content (via local SSH git)

`--workdir` is required for repository content copying. The CLI uses local SSH git operations to avoid Supabase timeout limits.

**Options:**

| Option | Description |
|--------|-------------|
| `--source-class` | Source class (ID, slug, or name) |
| `--target-class` | Target class (ID, slug, or name) |
| `--assignment` | Single assignment to copy (ID or slug) |
| `--schedule` | CSV file with assignment slugs/titles and date overrides |
| `--all` | Copy all assignments from the source class |
| `--workdir` | Local directory for git clones (required unless `--skip-repos`) |
| `--dry-run` | Preview changes without making them |
| `--skip-repos` | Skip git repository operations |
| `--skip-rubrics` | Skip rubric copying |
| `--skip-surveys` | Skip copying surveys linked to assignments |
| `--concurrency` | Parallel clone/fetch operations (1-8, default 4) |
| `--delay-ms` | Delay between clone batches in milliseconds |
| `--debug` | Enable server-side timing logs |

#### Schedule file format

Create a CSV with assignment slugs and new due dates:

```csv
slug,due_date
hw-01,2026-09-15T23:59:00
hw-02,2026-09-22T23:59:00
project-01,2026-10-01T23:59:00
```

A bare date (`2026-09-15`) means the end of that day in the class's time zone. Append a time (`2026-09-15T17:00`) for a specific time, or pass a timestamp with an offset to be explicit.

#### Delete an assignment

```bash
pawtograder assignments delete <assignment-id-or-slug> --class <class-id-or-slug>
```

Add `--force` to skip the confirmation prompt.

### Submissions

List submissions for an assignment, with autograder and total scores:

```bash
pawtograder submissions list --class cs3500-fall-2025 --assignment hw1
pawtograder submissions list --class cs3500-fall-2025 --assignment hw1 --json | jq '.submissions[] | select(.student_total_score == null)'
```

Options:
- `--include-non-submitters` — also list enrolled students who have not submitted
- `--limit <n>` — maximum rows to return (default 1000)
- `--json` — emit raw JSON instead of a formatted table

#### Import or sync comments

Batch import submission comments from autograder results:

```bash
# Add-only import
pawtograder submissions comments import \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --file batch-results.json \
  --author-profile-id <uuid>

# Sync (insert new + soft-delete missing)
pawtograder submissions comments sync \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --file batch-results.json \
  --rubric-part-id <id>
```

Use `--rubric-part-id` instead of `--author-profile-id` to attribute comments to review assignment assignees.

#### Import artifacts

Upload submission artifact blobs from a manifest:

```bash
pawtograder submissions artifacts import \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --file manifest.json \
  --batch-size 5
```

**Options:**

- `--overwrite` – Replace existing artifacts with the same name
- `--dry-run` – Preview counts only
- `--batch-size <n>` – Artifacts per API request (default 5)

The manifest is a JSON file with this structure:

```json
{
  "artifacts": [
    {
      "submission_id": 123,
      "name": "coverage-report",
      "data": {
        "format": "html",
        "display": "Coverage Report"
      },
      "content_base64": "PGh0bWw+Li4uPC9odG1sPg=="
    }
  ]
}
```

You can use `content_file` instead of `content_base64` to reference an external file:

```json
{
  "submission_id": 123,
  "name": "coverage-report",
  "data": { "format": "html", "display": "Coverage Report" },
  "content_file": "./reports/coverage-123.html"
}
```

### Reviews

List review assignments for an assignment:

```bash
pawtograder reviews list --class cs3500-fall-2025 --assignment hw1
pawtograder reviews list --class cs3500-fall-2025 --assignment hw1 --status pending
```

Options:
- `--rubric <type>` — restrict to one rubric: `grading`, `self_review`, `meta`, or a rubric ID
- `--assignee <name>` — filter by assignee name substring
- `--status <state>` — filter by completion: `pending`, `completed`, or `all` (default)
- `--limit <n>` — maximum rows (default 1000, max 5000)
- `--offset <n>` — skip this many rows (for pagination)
- `--json` — emit raw JSON

#### Assign grading work

`reviews assign` spreads submissions across active graders and instructors, balancing on how much each already holds. It honors `grading_conflicts` and never assigns someone their own submission.

```bash
# Preview a balanced split, one assignment per submission
pawtograder reviews assign \
  --class cs3500-fall-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
  --dry-run

# One assignment per rubric part instead
pawtograder reviews assign \
  --class cs3500-fall-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
  --by-part

# Restrict the grader pool
pawtograder reviews assign \
  --class cs3500-fall-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
  --grader <profile-id> \
  --grader <profile-id>
```

For allocations the round-robin cannot express, supply an explicit manifest:

```json
[{ "assignee_profile_id": "…", "submission_id": 123, "rubric_part_id": 45 }]
```

```bash
pawtograder reviews assign \
  --class cs3500-fall-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
  --file drafts.json
```

Re-running is safe: existing assignments are reused rather than duplicated.

A bare `--due-date` is the **end of that day in the class's time zone** — `2026-09-15` for a New York course means 23:59:59 Eastern, not midnight UTC. Append a time (`2026-09-15T17:00`) for something specific, or pass a timestamp with an offset to bypass the class zone entirely.

### Help requests

List office hours help requests:

```bash
pawtograder help-requests list --class cs3500-fall-2025
pawtograder help-requests list --class cs3500-fall-2025 --status active
```

Options:
- `--status <state>` — `open`, `in_progress`, `resolved`, `closed`, `active` (open or in_progress), or `all` (default)
- `--queue <id|name>` — restrict to one help queue
- `--limit <n>` — requests per page (default 100, max 1000)
- `--offset <n>` — skip this many requests
- `--json` — emit raw JSON

Close or resolve a help request (instructors only):

```bash
pawtograder help-requests close --id <id>
pawtograder help-requests close --id <id> --status resolved --resolution-status staff_helped
```

Options:
- `--status <state>` — `closed` (default) or `resolved`
- `--resolution-status` — `self_solved`, `staff_helped`, `peer_helped`, `no_time`, or `other`
- `--notes <text>` — resolution notes
- `--force` — close even if a live video call is running (the call will be left running)

### Discussions

List discussion topics with thread and question counts:

```bash
pawtograder discussions list --class cs3500-fall-2025
pawtograder discussions list --class cs3500-fall-2025 --json
```

### Rubrics

List rubrics for an assignment:

```bash
pawtograder rubrics list \
  --assignment <assignment-id-or-slug> \
  --class <class-id-or-slug>
```

Export a rubric to YAML:

```bash
pawtograder rubrics export \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --type grading \
  --output hw1-rubric.yml
```

Use `--strip-ids` to omit database IDs, producing a template suitable for importing into a different assignment rather than round-tripping the same one.

Import a rubric from YAML:

```bash
pawtograder rubrics import \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --file hw1-rubric.yml \
  --type grading
```

Use `--dry-run` to validate the YAML against the live rubric and see exactly what would change — including point changes, row removals, and reordering — without writing anything. Use `--verbose` to also print the parsed rubric tree.

Rubric types: `grading`, `self_review`, `meta`

### Surveys

Copy surveys between classes:

```bash
# Copy a single survey
pawtograder surveys copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --survey "Midterm Feedback"

# Copy all surveys
pawtograder surveys copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --all

# Link copied survey to a target assignment
pawtograder surveys copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --survey "HW1 Reflection" \
  --target-assignment hw1
```

### Flashcards

List flashcard decks:

```bash
pawtograder flashcards list --class <class-id-or-slug>
```

Copy flashcard decks between classes:

```bash
pawtograder flashcards copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --all
```

## Repository operations

Repository commands use local SSH git operations for reliability and performance.

<Warning>
Repository commands require SSH access to GitHub. Ensure your SSH keys are configured and you have access to the course organization.
</Warning>

### List repositories

```bash
pawtograder repos list --class <class-id-or-slug> --assignment <assignment-id-or-slug>
```

### Sync grade workflow

Synchronize `.github/workflows/grade.yml` from the handout repository to all student repositories:

```bash
pawtograder repos sync-grade-workflow \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --workdir /tmp/pawtograder-repos \
  --concurrency 2
```

This command:

1. Fetches the canonical `.github/workflows/grade.yml` from the handout repository.
2. Clones all student repositories locally via SSH.
3. Updates the workflow file in each repository.
4. Commits and pushes changes.

**Options:**

- `--dry-run` – Preview which repos would be updated
- `--concurrency <n>` – Parallel git operations (1-8, default 2)
- `--delay-ms <n>` – Delay between clone batches

### Cross-assignment copy

Copy files from source assignment repositories to target assignment repositories after the source due date. This is useful for sequential assignments where students build on previous work:

```bash
pawtograder repos copy-after-source-due \
  --class cs3500-spring-2026 \
  --source-assignment hw1 \
  --target-assignment hw2 \
  --workdir /tmp/pawtograder-repos \
  --concurrency 2
```

The CLI:

1. Verifies the source assignment due date has passed.
2. Clones source and target repositories for each student or group.
3. Copies files using rsync (excluding `.git`).
4. Commits and pushes to target repositories.

**Options:**

- `--dry-run` – Preview with `rsync -n` only
- `--mirror-delete` – Pass `rsync --delete` (still excludes `.git`)
- `--concurrency <n>` – Parallel operations (1-8, default 2)
- `--delay-ms <n>` – Delay between batches

## Bulk export

`submissions export` and `assessment export` stream large snapshots to a directory. Both gate personally identifying information behind an explicit `--i-understand-pii` flag and default to tokenized identifiers. See `--help` for the identity modes.

## JSON output

Every `list` command accepts `--json` to emit the raw API response instead of a formatted table. This is useful for scripting:

```bash
# Find students without a submission
pawtograder submissions list -c cs3500-fall-2026 -a hw-1 --json \
  | jq '.submissions[] | select(.student_total_score == null)'

# Count pending reviews
pawtograder reviews list -c cs3500-fall-2026 -a hw-1 --json \
  | jq '.summary.pending'
```

When `--json` is passed, all progress output is suppressed so stdout contains only the JSON payload.

## Common options

Most commands support these options:

- `--dry-run` – Preview changes without making them
- `--help` – Show command-specific help
- `--class` or `-c` – Specify a class by ID, slug, or name
- `--assignment` or `-a` – Specify an assignment by ID or slug
- `--json` – Emit raw JSON instead of a formatted table (list commands)

## Environment variables

All optional.

| Variable | Effect |
|----------|--------|
| `PAWTOGRADER_HTTP_TIMEOUT_MS` | Abort requests after this many milliseconds. Unset means no timeout. Raise it for large artifact imports. |
| `PAWTOGRADER_VERBOSE=1` | Log each request and its timing to stderr. |
| `DEBUG=1` | Same as above, plus stack traces on unexpected errors. |

The CLI does not read `.env` files.

## API token scopes

CLI tokens require specific scopes:

- `cli:read` – Read-only operations (list, show, export)
- `cli:write` – Write operations (copy, import, delete, assign, close)

Create tokens with appropriate scopes in the web UI under **Settings → API Tokens**.

## Common workflows

### Semester rollover

Copy an entire course to a new semester:

```bash
# 1. Copy all assignments with new dates
pawtograder assignments copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --schedule spring-2026-schedule.csv \
  --workdir ./repos

# 2. Copy surveys
pawtograder surveys copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --all

# 3. Copy flashcards
pawtograder flashcards copy \
  --source-class cs3500-fall-2025 \
  --target-class cs3500-spring-2026 \
  --all
```

### Bulk rubric updates

```bash
# 1. Export the current rubric
pawtograder rubrics export \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --output hw1-rubric.yml

# 2. Edit hw1-rubric.yml locally

# 3. Preview changes
pawtograder rubrics import \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --file hw1-rubric.yml \
  --dry-run

# 4. Apply changes
pawtograder rubrics import \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --file hw1-rubric.yml
```

### Assign grading and track progress

```bash
# Assign reviews balanced across all graders
pawtograder reviews assign \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
  --dry-run

# Check completion
pawtograder reviews list \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --status pending \
  --json | jq '.summary'
```

### Repository maintenance

```bash
# Update grade workflows across all student repos
pawtograder repos sync-grade-workflow \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --workdir ./repos \
  --dry-run

# Copy starter code from hw1 to hw2 after hw1 due date
pawtograder repos copy-after-source-due \
  --class cs3500-spring-2026 \
  --source-assignment hw1 \
  --target-assignment hw2 \
  --workdir ./repos \
  --dry-run
```

## Best practices

1. **Use dry-run first.** Preview changes with `--dry-run` before executing.
2. **Export backups.** Export rubrics before importing new versions.
3. **Batch size.** For artifact imports, reduce `--batch-size` if requests time out.
4. **Dedicated workdir.** Use a dedicated directory for `--workdir` to avoid conflicts.
5. **Concurrency.** Start with the default and adjust based on network performance.

## Troubleshooting

### Authentication errors

If you see "Not logged in" or "Authentication failed":

1. Run `pawtograder login` to re-authenticate.
2. Verify your token is valid in the web UI (**Settings → API Tokens**).
3. Ensure your token has the correct scopes (`cli:read` or `cli:write`).
4. For self-hosted deployments, confirm `--url` points to the API gateway origin (`https://api.<hostname>`), not the web host.

Common HTTP errors:

- **401 Unauthorized** – Token expired or revoked. Log in again.
- **403 Forbidden** – Token lacks required scopes.
- **504 Gateway Timeout** – Operation took too long. Increase the timeout or use smaller batches.

### Command not found

If using the npm package, ensure it is installed: `npm install -g @pawtograder/cli`. If running from the repository, run from the repository root with `npm run cli -- <command>`.

### Timeout errors

For large operations (copying many assignments, importing large artifacts):

1. Set a longer timeout: `PAWTOGRADER_HTTP_TIMEOUT_MS=600000 pawtograder ...`
2. Use smaller batch sizes (for example, `--batch-size 1` for artifact imports).
3. Use `--dry-run` first to validate before running the full operation.

### Repository clone failures

If SSH git operations fail:

1. Verify SSH keys are configured: `ssh -T git@github.com`
2. Check that you're a member of the course's GitHub organization.
3. Retry with lower `--concurrency` (try 2 or 1).
4. Use `--delay-ms 1000` to add delays between batches.

## Getting help

For detailed help on any command:

```bash
pawtograder <command> --help
```

For example:

```bash
pawtograder assignments --help
pawtograder assignments copy --help
pawtograder reviews assign --help
```
