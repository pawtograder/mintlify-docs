...
The CLI also ships inside the platform repository and runs via npm scripts:
...
All commands accept `--help` for detailed options. List commands accept `--json` for machine-readable output.
...
Add `--strip-ids` to export a template that creates all rows as new on import (useful for copying a rubric structure to a different assignment).
...
Use `--dry-run` to validate the import against the live rubric and see exactly what would change, without writing anything.
...
```bash
pawtograder help-requests close --id 42 --status resolved --resolution-status staff_helped
```
...
Create review assignments balanced round-robin across active graders and instructors. The allocator honors grading conflicts and never assigns someone their own submission:
...
The manifest is a JSON array of `{ "assignee_profile_id": "...", "submission_id": 123, "rubric_part_id": null }` objects. Explicit manifests are validated against the same grading-conflict and self-review exclusions as the round-robin allocator.
...
- `--json` – Emit the raw JSON response instead of a formatted table (list commands)
...
| `PAWTOGRADER_VERBOSE` | Set to `1` to log each request and its timing to stderr. |
...
```bash
# Preview a balanced grading split
pawtograder reviews assign \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
...
If the CLI returns an HTML page error, you are pointing at the web host instead of the API gateway. Use `pawtograder login --url https://api.<hostname>` to point at the correct endpoint.

### Command not found

If using the npm package, run `npm install -g @pawtograder/cli`. If running from the repository, run from the platform repository root and make sure dependencies are installed with `npm install`.
