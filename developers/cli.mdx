...
The CLI source lives in `cli/` and is shared with the unit tests. Run it from the repository root:
...
You are prompted to enter your API token. The token is stored in `~/.pawtograder/credentials.json`.
...
Use `--dry-run` to preview the import without making changes. The dry run validates the YAML against the live rubric and reports what would change.
...
List submissions for an assignment (roster with active submission, autograder and total scores):
...
Assign grading work (balanced round-robin across active graders and instructors, honoring `grading_conflicts` and never assigning someone their own submission):
...
A bare `--due-date` (e.g. `2026-09-15`) means end of that day in the class's time zone. Re-running is safe: existing assignments are reused rather than duplicated.
...
| `DEBUG` | Set to `1` to enable verbose HTTP logging plus stack traces on unexpected errors. |
...
4. For self-hosted deployments, ensure `--url` points at the API gateway origin (e.g. `https://api.your-school.edu`), not the web host. Passing the web host produces an HTML response and the CLI will tell you so.
...
If using the npm package, ensure it is installed: `npm install -g @pawtograder/cli`.

If running from the repository, run from the platform repository root and make sure dependencies are installed with `npm install`.
