...
The CLI is published to npm as [`@pawtograder/cli`](https://www.npmjs.com/package/@pawtograder/cli). Run it without installing:
...
The CLI also ships inside the platform repository. Run it from the repository root:
...
For custom deployments, point `--url` at your deployment's **API gateway origin** (not the web host):
...
Add `--strip-ids` to export a template without database IDs, so importing it into another assignment creates all rows fresh:
...
pawtograder rubrics export \
  ...
  --type grading \
  --strip-ids \
  --output hw1-rubric-template.yml
```
...
Use `--dry-run` to validate the import and preview what would change without making any modifications. The dry run runs against the live rubric, so it catches enum errors, reference resolution failures, and structural changes (point changes, row deletions) that a local file check cannot.
...
pawtograder submissions list \
  --class cs3500-fall-2025 \
  --assignment hw1 \
  --json | jq '.submissions[] | select(.autograder_score == null)'
```
...
Filter by status (`open`, `in_progress`, `resolved`, `closed`, or `active` for open + in-progress):
...
```bash
pawtograder help-requests close --id 42 --status resolved \
...
# Create one assignment per rubric part
pawtograder reviews assign \
  --class cs3500-fall-2025 \
  --assignment hw1 \
  --due-date 2026-09-15 \
...
The allocator honors `grading_conflicts` and never assigns someone their own submission. A bare `--due-date` (e.g. `2026-09-15`) means end of that day in the class's time zone. Re-running is safe: existing assignments are reused rather than duplicated.
