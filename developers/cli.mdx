...
The CLI is published to npm as `@pawtograder/cli`:
...
The CLI source lives at `cli/` in the platform repository and is shared with the unit tests and the published package. Run it from the repository root:
...
The CLI is a yargs program defined in `cli/program.ts`. Two entry points wrap it:
...
The published binary deliberately does not read `.env` files: credentials live in `~/.pawtograder/credentials.json`, and a globally installed tool should not absorb the environment of whatever directory it is invoked from.
...
```bash
pawtograder login --url https://api.your-instance.edu
```
...
Re-running is safe: existing assignments are reused rather than duplicated. A bare `--due-date` is the **end of that day in the class's time zone** — `2026-09-15` for a New York course means 23:59:59 Eastern, not midnight UTC.
...
Use `--strip-ids` to export a template that imports as entirely new rows (useful for copying a rubric structure to a new assignment).
...
`--dry-run` validates the YAML against the live rubric and reports what would change (insertions, updates, deletions, point changes) without writing anything.
...
Every `list` command accepts `--json` for machine-readable output. When `--json` is set, all progress logging is suppressed so stdout contains only the JSON payload:
...
pawtograder classes list --json | jq '.classes[].slug'
```
...
| `DEBUG=1` | Enable verbose HTTP logging and stack traces on unexpected errors. |

The published CLI does not read `.env` files. Set these variables in your shell or CI environment.
...
The CLI source at `cli/` is shared between `npm run cli`, the unit tests, and the published `@pawtograder/cli` package. The package is built with esbuild, which resolves the `@/cli/*` path alias and bundles all internal modules into a single file. The three runtime dependencies (`yargs`, `yaml`, `csv-parse`) stay external.

Releases are triggered by pushing a `cli-v*` tag. The CI workflow builds, smoke-tests, and publishes with provenance attestation. See `packages/cli/RELEASING.md` for the full release process.
...
If using the npm package, ensure it is installed: `npm install -g @pawtograder/cli`. If running from source, run from the platform repository root with `npm run cli`.
