...
The CLI also ships inside the platform repository and runs via npm scripts. Run it from the repository root:
...
<Note>
When using the in-repo `npm run cli` entry point, substitute `npm run cli --` for `pawtograder` in all examples below.
</Note>
...
All list commands accept `--json` to emit the raw API response for piping into `jq` or other tools.
...
Use `--strip-ids` to export a template that creates all rows as new on import (useful for copying a rubric structure to a new assignment).
...
Use `--dry-run` to validate against the live rubric and preview the full plan of insertions, updates, deletions, and point changes before any data is written.
...
List submissions for an assignment, including active submission IDs, autograder scores, total scores, and grader assignments:
...
Pass `--include-non-submitters` to also list enrolled students who have not submitted.
...
Filter by status (`open`, `in_progress`, `resolved`, `closed`, or `active` for open and in-progress combined):
...
List discussion board topics with thread, question, and unanswered-question counts:
...
List review assignments with completion state and scores:
...
Filter by status (`pending`, `completed`, or `all`) or by assignee:
...
Create review assignments balanced round-robin across graders, honoring grading conflicts and never assigning someone their own submission:
...
A bare `--due-date` (for example `2026-09-15`) means end of that day in the class's time zone. Append a time (`2026-09-15T17:00`) for a specific time.

**Options:**

| Option | Description |
|--------|-------------|
| `--by-part` | Create one assignment per rubric part instead of one per submission |
| `--grader <id>` | Restrict the pool to specific grader profile IDs (repeatable) |
| `--include-non-submitters` | Also assign placeholder stubs for students who did not submit |
| `--file <manifest.json>` | Supply explicit `{assignee_profile_id, submission_id, rubric_part_id}` triples instead of round-robin |
...
# 3. Preview changes (validates against the live rubric on the server)
pawtograder rubrics import \
  --assignment hw1 \
  --class cs3500-spring-2026 \
  --file hw1-rubric.yml \
...
```bash
# Preview a balanced split across all graders
pawtograder reviews assign \
  --class cs3500-spring-2026 \
  --assignment hw1 \
  --due-date 2026-09-15 \
...
If the CLI reports that the API returned an HTML page, you are pointing at the web host instead of the API gateway. Use `pawtograder login --url https://api.<hostname>` to point at the correct endpoint.

### Command not found

If using the npm-installed binary, ensure Node.js 20 or later is installed and run `npm install -g @pawtograder/cli`.

If using the in-repo entry point, run from the platform repository root and make sure dependencies are installed with `npm install`.
