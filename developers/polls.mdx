...
- `deactivates_at`: Intended auto-deactivation timestamp (1 hour from when poll goes live). Written but never read — see [Auto-Deactivation](#auto-deactivation-not-currently-wired-up).
...
### Auto-Deactivation (not currently wired up)

The `set_poll_deactivates_at` trigger stamps `deactivates_at` one hour ahead whenever a poll goes live, and a function exists to act on it:
...
<Warning>
Nothing calls this function. It is never scheduled with `pg_cron` and has no caller anywhere in the codebase, so **polls do not currently close on their own** — `deactivates_at` is written and then never read. A poll stays live until someone closes it explicitly. Scheduling it (`SELECT cron.schedule('deactivate-expired-polls', '0 * * * *', 'SELECT deactivate_expired_polls()')`) would be the change needed to make the one-hour expiry real.
</Warning>
