...
```bash
# Export all assignments (opaque mode — safe default)
...
- `manifest.json` — metadata about the export run
- `subjects.json` — one record per enrolled student (tokenized or raw)
...
<Warning>
The deanonymize command writes real student names, emails, and SIS IDs to disk. Handle the output file accordingly and delete it when no longer needed.
</Warning>
...
- `--i-understand-pii`: Required for `--identity raw` and `assessment deanonymize`
...
- `--with-test-output`: Include autograder test output (can be large)
