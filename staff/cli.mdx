...
Import and export rubrics in YAML format. Cross-rubric check references (e.g. a grading-review check that references a self-review check) are preserved on both export and import.
...
```bash
# Export with opaque (per-run) subject tokens (default)
...
- `assignments/<slug>/rubric.json`: Rubric tree
- `assignments/<slug>/submissions.json`: Active submissions
- `assignments/<slug>/scores.json`: Rubric check results
- `assignments/<slug>/tests.json`: Autograder test results
- `assignments/<slug>/hints.json`: Hint feedback
...
| `hash` | HMAC tokens derived from `--salt`; joinable across runs with the same salt |
| `raw` | Real student IDs, emails, and names; requires `--i-understand-pii` |

#### Deanonymize (hash mode only)

After a `--identity hash` export, generate a CSV mapping subject tokens back to real student identifiers:
