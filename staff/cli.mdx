...
```bash
# Export assessment data for a class (default: opaque identity mode)
npm run cli -- assessment export --class <identifier>

# Export with deterministic anonymization (joinable across runs)
npm run cli -- assessment export \
  --class <identifier> \
  --identity hash \
  --salt <at-least-16-char-string>

# Export with real student identifiers (requires acknowledgement)
npm run cli -- assessment export \
  --class <identifier> \
  --identity raw \
  --i-understand-pii
...
| `hash` | Deterministic HMAC tokens derived from `--salt`. Tokens are joinable across runs using the same salt. |
| `raw` | Real student IDs, emails, and names. Requires `--i-understand-pii`. |

#### Export options

- `--class <id>`: Class ID, slug, or name (required)
- `--identity <mode>`: `opaque` (default), `hash`, or `raw`
- `--salt <string>`: Salt for `hash` mode (minimum 16 characters)
...
assessment-export-<class>-<timestamp>/
  ...
  subjects.json          # Anonymized student roster
  ...
  assignments/
    <slug>/
      ...
      scores.json        # Rubric check scores from final reviews
...
If you exported with `--identity hash`, you can later produce a CSV mapping subject tokens back to real student identifiers:
...
This produces a CSV with columns: `subject_token`, `name`, `email`, `sis_user_id`, `class_section`, `lab_section`.
