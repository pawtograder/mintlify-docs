...
```bash
# Default (opaque tokens, all assignments and gradebook columns)
npm run cli -- assessment export --class cs3500-spring-2026

# Hash mode — tokens are stable across runs with the same salt
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --identity hash \
  --salt "my-secret-salt-at-least-16-chars"

# Export only specific assignments (supports globs)
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --assignment "hw-*" \
  --assignment lab1

# Include autograder test output (truncated to 4096 bytes per test by default)
npm run cli -- assessment export \
  --class cs3500-spring-2026 \
  --with-test-output \
...
The export writes a directory tree:

```
assessment-export-<class>-<timestamp>/
  manifest.json
  subjects.json        # one entry per enrolled student
...
After an export with `--identity hash`, you can produce a CSV that maps subject tokens back to real student identifiers:
...
| `--assignment, -a` | Assignment selector (ID, slug, or glob). Repeatable. Omit for all. |
...
| `--with-test-output` | Include autograder test output (off by default) |
