...
# Deanonymize: map subject tokens back to real student identifiers
...
| Mode | Description |
|------|-------------|
| `opaque` (default) | Random salt generated per run. Tokens are only joinable within a single export run. |
| `hash` | Deterministic HMAC tokens derived from `--salt`. Tokens are joinable across runs that use the same salt. |
| `raw` | Real student IDs, emails, and names. Requires `--i-understand-pii`. |

#### Output structure

The export writes to a timestamped directory (e.g. `assessment-export-cs3500-fall-2025-20260520-120000/`):
...
#### Assignment and gradebook column selectors

Use `--assignment` and `--gradebook-column` to filter the export. Each selector can be:
- A numeric ID
- An exact slug
- A glob pattern (e.g. `hw-*`, `lab?`)

Unmatched selectors are reported as warnings in the output.
...
<Warning>
The deanonymization CSV contains real student PII. Store it securely and handle it according to your institution's data policies.
</Warning>
