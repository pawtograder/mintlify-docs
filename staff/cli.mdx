...
Export a privacy-controlled snapshot of class assessment data — rubrics, scores, autograder test results, hint feedback, and gradebook columns — for downstream analysis.

#### Identity modes

The export supports three identity modes to control how student identifiers appear in the output:

| Mode | Description |
|---|---|
| `opaque` (default) | Random per-run salt; tokens are only joinable within a single export run |
| `hash` | Deterministic HMAC tokens derived from a caller-supplied `--salt`; joinable across runs with the same salt |
| `raw` | Real student IDs, emails, and names; requires `--i-understand-pii` |

#### Export assessment data

```bash
# Export all assignments (opaque identity, default)
npm run cli -- assessment export --class <identifier>

# Export with deterministic tokens (joinable across runs)
npm run cli -- assessment export \
  --class <identifier> \
  --identity hash \
  --salt "my-secret-salt-at-least-16-chars"

# Export with real student identifiers
npm run cli -- assessment export \
  --class <identifier> \
  --identity raw \
  --i-understand-pii

# Export specific assignments only
npm run cli -- assessment export \
  --class <identifier> \
  --assignment hw-1 \
  --assignment hw-2
...
# Include autograder test output (truncated to 4096 bytes by default)
npm run cli -- assessment export \
  --class <identifier> \
  --with-test-output \
  --test-output-max-bytes 8192

# Control parallel streams (1–8, default 4)
npm run cli -- assessment export \
  --class <identifier> \
  --concurrency 2
```
...
- `--class, -c`: Class ID, slug, or name (required)
- `--identity`: `opaque` | `hash` | `raw` (default: `opaque`)
- `--salt`: Salt for `hash` identity mode (required when `--identity hash`; minimum 16 characters)
- `--i-understand-pii`: Required when `--identity raw`; acknowledges that real student data will be written to disk
- `--output, -o`: Output directory (default: `./assessment-export-<class>-<timestamp>`)
- `--assignment, -a`: Assignment selector — ID, slug, or glob (repeatable; omit to export all)
- `--gradebook-column`: Gradebook column selector — ID, slug, or glob (repeatable; omit to export all)
- `--concurrency`: Parallel per-assignment streams, 1–8 (default: 4)
- `--with-test-output`: Include autograder test output (off by default; outputs can be large)
- `--test-output-max-bytes`: Per-test output cap when `--with-test-output` is set (default: 4096)

#### Output structure

The export writes a directory with the following layout:

```
assessment-export-<class>-<timestamp>/
  manifest.json          # Export metadata and grand totals
  subjects.json          # One record per enrolled student
  sections.json          # Class and lab sections
  assignments.json       # Assignment catalog
  gradebook-columns.json # Gradebook column catalog
  assignments/
    <slug>/
      manifest.json      # Per-assignment metadata
      rubric.json        # Rubric tree (parts, criteria, checks)
      autograder.json    # Autograder config and test catalog
      submissions.json   # Active submissions
      scores.json        # Rubric check results from final grading review
      tests.json         # Autograder test results
      hints.json         # Hint feedback
      groups.json        # Group membership (group assignments only)
  gradebook/
    scores.json          # Instructor-side gradebook cells
```

All files are written with restrictive permissions (`0o600`) to protect student data.

#### Deanonymize (map tokens back to students)

When you used `--identity hash`, you can produce a CSV that maps subject tokens back to real student identifiers using the same salt:

```bash
npm run cli -- assessment deanonymize \
  --class <identifier> \
  --salt "my-secret-salt-at-least-16-chars" \
  --i-understand-pii
```
...
Options:
- `--class, -c`: Class ID, slug, or name (required)
- `--salt`: Must match the salt used in the original export run (required; minimum 16 characters)
- `--i-understand-pii`: Required; acknowledges that real student names, emails, and SIS IDs will be written to disk
- `--output, -o`: Output CSV file path (default: `./<class>-roster-<timestamp>.csv`)

The output CSV has columns: `subject_token`, `name`, `email`, `sis_user_id`, `class_section`, `lab_section`.

<Warning>
The deanonymize command writes real student PII to disk. Handle the output file with appropriate care and delete it when no longer needed.
</Warning>
