...
Export a privacy-controlled snapshot of class assessment data — rubrics, final grading review scores, autograder test results, hint feedback, and gradebook columns.

```bash
# Export with opaque (per-run random) identifiers (default)
npm run cli -- assessment export --class <identifier>

# Export with deterministic hash identifiers (joinable across runs)
...
- `sections.json` — class and lab section metadata
- `assignments.json` — assignment catalog
- `gradebook-columns.json` — gradebook column definitions
- `assignments/<slug>/` — per-assignment data (rubric, autograder config, submissions, scores, tests, hints)
...
| `hash` | HMAC of student ID with a caller-supplied `--salt`; tokens are deterministic and joinable across runs with the same salt |
| `raw` | Real student IDs, emails, and names; requires `--i-understand-pii` |

#### Deanonymize (hash mode only)

After exporting with `--identity hash`, you can produce a CSV mapping subject tokens back to real student identifiers:
