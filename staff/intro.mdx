...
3. An orange banner appears at the top of every page: **"Viewing as [student name] — read only."**

While in student view:

- You see the course through that student's eyes: their submissions, their grades (only released grades), their discussion posts, their office hours requests.
- All write actions are disabled. Buttons that would create or modify data are hidden or shown as read-only.
- The rubric sidebar shows only what the student can see (respecting `student_visibility` and release state).
- Autograder output shows only student-visible tests; instructor-only debug output is hidden.
- Flashcard progress is not loaded (to avoid mixing your progress with the student's).

### Exiting student view

Click **Exit student view** in the orange banner. You return to your normal instructor view immediately.

<Note>
Student view is read-only and purely presentational. No data is written under the student's identity. RLS (row-level security) still runs under your real instructor credentials, so the view is a presentation layer, not a true impersonation.
</Note>
...
The default time zone is America/New_York (Eastern Time), but you can change this to match your course's location or preferred time zone. See [Course Settings](/staff/course-settings#time-zone-configuration) for configuration details.
