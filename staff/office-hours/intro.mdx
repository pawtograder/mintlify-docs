...
Queues can be marked as active or inactive. **TAs and students only see active queues** on their dashboards, ensuring a clean interface focused on currently available help options. Inactive queues remain visible in the Queue Management interface for instructors to reactivate as needed.

When all queues are inactive, TAs will see an empty state with a helpful message explaining that no queues are currently active.
