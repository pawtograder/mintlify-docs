...
The office hours dashboard shows only **active queues** to teaching assistants and staff members. Inactive queues are hidden from the main dashboard but remain accessible in the Queue Management interface for instructors. This ensures staff focus on currently available help queues while students see the same set of active queues.

When all queues are inactive, staff will see an empty state with a helpful message.
