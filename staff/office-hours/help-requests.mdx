...
<Note>
Referencing a submission or a file forces the request private. Removing the last code reference does **not** make the request public again — privacy can only escalate. Only course staff can make a private request public. The picker lists your own submissions rather than the student's, so it is more useful for reviewing and removing references than for adding them.
</Note>
