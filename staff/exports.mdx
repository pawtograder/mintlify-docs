...
snapshot of an assignment's rubric, scores, autograder test results, hint
feedback, and gradebook columns. Each run writes a directory of JSON files:

| File | Contents |
|---|---|
| `manifest.json` | Run metadata, identity mode, totals |
| `rubric.json` | Rubric hierarchy (parts, criteria, checks) |
| `autograder.json` | Autograder config and test definitions |
| `submissions.json` | One record per active (or all) submission |
...
| `instructor-build-output.json` | Hidden grader build logs (opt-in via `--with-instructor-build-output`) |
| `gradebook/scores.json` | Private gradebook column values |

### Identity modes

Student identifiers are pseudonymized by default. Three modes are available:

| Mode | Description |
|---|---|
| `opaque` (default) | Random per-run salt; tokens are stable within one run but not reproducible across runs |
| `hash` | Deterministic tokens from a `--salt` you supply; joinable across runs on the same deployment |
