...
The Pawtograder CLI provides a privacy-controlled bulk export of rubrics, scores, autograder test results, Feedbot hint feedback, and gradebook data. Run:

```bash
pawtograder assessment export -c <class>
```
...
| Mode | Description |
|---|---|
| `opaque` (default) | Random per-run salt + server pepper. Tokens are stable within one run but cannot be reproduced later. |
| `hash` | Deterministic from `--salt` + server pepper. Same salt on the same deployment produces the same tokens across runs (joinable). |
...
| `submissions.json` | One record per submission (active by default). |
...
| `error-pin-engagement.json` | Per-student read/like status for discussion posts pinned to autograder errors. |
| `instructor-build-output.json` | Hidden build logs (only when `--with-instructor-build-output`). |
| `gradebook/scores.json` | Private gradebook cell values. |

### Submissions export

To export submission source files:

```bash
pawtograder submissions export -c <class> [--include-file 'src/**'] [--exclude-file 'build/**']
```

Supports the same identity modes as assessment export. Output includes `submissions.json` (metadata) and `files.json` (file contents) per assignment, plus a root `manifest.json`.
