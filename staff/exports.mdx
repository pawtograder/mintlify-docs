...
## Grader Hint Feedback

**Where**: Manage course → Assignments → *assignment* → **Export** (with the "Include grader hint feedback" option enabled).

When exporting submission data, you can include linked grader hint feedback — the AI-generated hints that students requested during the assignment, along with any feedback students provided on those hints. This allows instructors to review hint quality and student engagement with AI assistance.

The export includes hint text, student feedback ratings, and the associated test or rubric check that triggered the hint request.
