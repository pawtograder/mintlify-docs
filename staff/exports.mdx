...
The Pawtograder CLI (`pawtograder assessment export`) produces a privacy-controlled snapshot of rubric scores, autograder test results, Feedbot hint feedback, and gradebook columns for a class. Run it from a terminal with the CLI installed.

```sh
pawtograder assessment export --class <id-or-slug> --output ./export
```

The export writes one subdirectory per assignment containing:
...
| `submissions.json` | One row per active submission (or all attempts with `--all-submissions`). |
| `scores.json` | Rubric check application records from the final grading review. |
...
Student identifiers are pseudonymized by default. Use `--identity` to control the mode:

| Mode | Description |
|---|---|
| `opaque` (default) | Random per-run salt; tokens are only joinable within one dump. |
| `hash` | Deterministic from `--salt`; tokens are joinable across dumps on the same deployment. |
...
- `--assignment hw-*` — export only assignments matching a glob.
