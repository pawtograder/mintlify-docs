...
**Where**: Assignment → Dashboard → Rubric breakdown → **Export CSV**.

Exports the rubric check application statistics for the current cohort and
visualization. Columns vary by visualization mode:

- **Bars / Table**: Criterion, Check, Option (blank for non-choice checks), Applied count, Cohort size, Percent
- **By section**: Criterion, Check, Section, Applied count, Cohort size, Percent

The export respects the active cohort filter and the selected rubric items.

Filename: `rubric-report-assignment-<assignment_id>-<viz>.csv`.
