...
The Pawtograder CLI provides a richer, privacy-controlled export of
assessment data (rubrics, scores, autograder test results, Feedbot hint
feedback, and gradebook columns). See the [CLI documentation](/developers/cli)
for installation and authentication.

```bash
pawtograder assessment export --class <class-id> --output ./my-export
```
...
| Mode | Description |
|---|---|
| `opaque` (default) | Random per-run salt; tokens are only joinable within a single dump. |
| `hash` | Deterministic HMAC using `--salt`; tokens are joinable across dumps on the same deployment. |
...
| File | Contents |
|---|---|
| `manifest.json` | Dump metadata, identity mode, and grand totals. |
| `rubric.json` | Rubric tree (parts, criteria, checks). |
| `autograder.json` | Autograder configuration and test definitions. |
| `submissions.json` | One row per submission (active only by default). |
| `scores.json` | Rubric check scores from the final grading review. |
| `tests.json` | Autograder test results. |
| `hints.json` | Feedbot hint feedback (student ratings and LLM responses). |
| `error-pin-engagement.json` | Per-student engagement with discussion posts pinned to autograder errors. |

## CLI Submissions Export

Export raw submission files and metadata with the same identity modes as the assessment export.

```bash
pawtograder submissions export --class <class-id> --output ./submissions
```
...
| Option | Description |
|---|---|
| `--assignment <slug>` | Export specific assignments (repeatable, supports globs). |
| `--all-submissions` | Include every attempt, not just the active submission. |
...
| File | Contents |
|---|---|
| `manifest.json` | Dump metadata and totals. |
| `submissions.json` / `submissions.csv` | One row per submission. |
| `files.json` / `files.csv` | One row per file per submission. |
