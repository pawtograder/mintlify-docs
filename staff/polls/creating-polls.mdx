...
A poll stays live until you close it. Close it from the poll management view or the response dashboard when you are done collecting responses.

<Warning>
Polls do not close on their own. Publishing a poll stamps an internal one-hour expiry timestamp, but nothing acts on it, so a poll left open stays open — including after class ends. Close polls you are finished with, especially ones used for attendance.
</Warning>
...
- **Open Poll**: Makes a closed poll live again
- **Close Poll**: Closes a poll, so it stops accepting responses
