...
1. Open the root thread you want to mark as a duplicate.
...
- A banner is shown on the merged post indicating the original subject and who merged it.
- The duplicate author receives an in-app notification (and email when the queue processor runs) with a link to their post under the canonical thread.
- The duplicate thread disappears from the discussion feed.

<Note>
Only root-level threads can be marked as duplicates. You cannot mark a reply as a duplicate.
</Note>
