...
The duplicate thread and all of its replies are moved under the original thread. A banner is shown on the merged post indicating the former subject and who merged it. The author of the duplicate thread receives an in-app notification (and email) with a link to their post in the original thread.

<Note>
Only root-level threads can be marked as duplicates. Once merged, the duplicate thread no longer appears in the discussion feed.
</Note>
