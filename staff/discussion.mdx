...
When students post the same question multiple times, graders and instructors can merge a duplicate thread into the original. This keeps the discussion board organized and ensures students find the canonical answer.

### How to merge a duplicate
...
- The duplicate thread and all its replies move under the original thread as replies.
- A banner on the merged post shows the former subject and the staff member who merged it.
- The duplicate thread disappears from the discussion feed.
- The author of the duplicate post receives an in-app notification (and email) with a link to their post under the original thread.

<Note>
Only root-level threads can be marked as duplicates. A thread that has already been merged cannot be marked as a duplicate again.
</Note>
