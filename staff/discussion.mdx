...
When two threads ask the same question, graders and instructors can merge the duplicate into the original instead of answering it twice.

1. Open the duplicate thread and choose **Mark Duplicate**.
2. Search for the original thread by title or by post number (e.g. `#42`).
3. Choose **Merge into original**.

After merging:

- The duplicate's replies are moved under the original thread.
- A banner on the original shows the former subject and who merged it.
- The duplicate author receives an in-app notification with a deep link to their post in the canonical thread.
- The duplicate is removed from the feed.

Only root threads can be marked as duplicates, and a merged thread cannot be merged again.
