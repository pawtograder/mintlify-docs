...
## Marking Duplicate Threads

When students post the same question multiple times, graders and instructors can merge the duplicate thread into the original. This keeps the discussion board organized and ensures all replies are consolidated in one place.

### How to mark a thread as a duplicate

1. Open the duplicate root thread (the one you want to merge away)
...
After merging:
- The duplicate thread and all its replies are moved under the original thread as replies
- A banner on the merged post shows the former subject and the name of the staff member who merged it
- The duplicate author receives an in-app notification (and email) with a link to their post in the original thread
- The duplicate thread no longer appears in the discussion feed

<Note>
Only root-level threads can be marked as duplicates. The **Mark Duplicate** button is only visible to graders and instructors, and only on threads that have not already been merged.
</Note>
