...
Graders and instructors can merge a duplicate discussion thread into another thread. This is useful when students post the same question multiple times, or when a new post is clearly a duplicate of an existing one.

To mark a thread as a duplicate:

1. Open the thread you want to mark as a duplicate.
2. Click **Mark Duplicate** in the thread actions.
...
- A banner appears on the merged post showing its former subject and who merged it.
- The author of the duplicate thread receives an in-app notification (and email, if configured) with a link to their post in the original thread.
- The duplicate thread no longer appears in the discussion feed.

<Note>
Only root-level threads (not replies) can be marked as duplicates. A thread that has already been merged cannot be marked as a duplicate again.
</Note>
