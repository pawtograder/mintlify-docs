...
Graders and instructors can merge a duplicate discussion thread into another thread. This is useful when students post the same question multiple times, or when a question has already been answered in another thread.

To mark a thread as a duplicate:

1. Open the root thread you want to merge (the duplicate).
2. Click **Mark Duplicate** in the thread actions toolbar.
3. Search for the original thread by title or post number (e.g. `#42`).
4. Select the original thread from the results list.
5. Click **Merge into original**.

When a thread is merged:

- The duplicate thread and all of its replies are moved under the original thread as replies.
- A banner is displayed on the merged post showing the original subject and who merged it.
- The author of the duplicate thread receives an in-app notification (and email) with a link to their post under the canonical thread.
- The duplicate thread is removed from the discussion feed so it no longer appears as a separate root thread.

<Note>
Only root threads can be marked as duplicates. Replies cannot be merged this way.
</Note>
