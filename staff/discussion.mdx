...
The duplicate thread and all of its replies are moved under the original thread. A banner is displayed on the merged post showing the original subject and who merged it. The author of the duplicate thread receives an in-app notification (and email) with a link to their post in the canonical thread.
