...
Graders and instructors can merge a duplicate discussion thread into another thread. This is useful when students post the same question multiple times or when a question is better answered in an existing thread.

### How to mark a thread as a duplicate

1. Navigate to the root thread you want to mark as a duplicate.
2. Click **Mark Duplicate** in the thread actions toolbar.
3. Search for the original thread by title or post number (e.g. `#42` or `42`).
4. Select the original thread from the results.
5. Click **Merge into original**.

The duplicate thread and all of its replies are moved under the original thread as replies. A banner is shown on the merged post indicating the former subject and who merged it.

### What happens after merging

- The duplicate thread disappears from the discussion feed.
- The duplicate thread's replies appear under the original thread.
- The author of the duplicate thread receives an in-app notification (and email when the notification queue runs) with a link to their merged post.
- A banner on the merged post reads: *"Originally [former subject] — [staff name] marked this as a duplicate and merged it here."*

<Note>
Only root-level threads can be marked as duplicates. Individual replies cannot be merged.
</Note>
