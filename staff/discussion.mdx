...
1. Open the duplicate thread and click **Mark Duplicate** in the thread actions.
2. Search for the original thread by title or post number (e.g. `#42`).
3. Select the original thread and click **Merge into original**.

The duplicate thread and all of its replies are moved under the original thread as replies. A banner on the merged post shows the former subject and who merged it. The duplicate author receives an in-app notification (and email when the queue processor runs) with a deep link to their merged post under the canonical thread.

Duplicate-marked threads no longer appear in the discussion feed.
