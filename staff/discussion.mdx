...
Graders and instructors can merge a duplicate discussion thread into another thread. When a thread is marked as a duplicate:
...
- The duplicate author receives an in-app notification with a deep link to their merged post under the canonical thread.
...
Only root-level threads can be marked as duplicates. A thread cannot be marked as a duplicate of itself.
