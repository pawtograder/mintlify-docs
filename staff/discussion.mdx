...
When students post the same question multiple times, graders and instructors can merge a duplicate thread into the original. This keeps the discussion board organized and ensures all replies are consolidated in one place.

To mark a thread as a duplicate:

1. Open the duplicate root thread.
2. Click **Mark Duplicate** in the thread actions (visible to graders and instructors on root threads that haven't already been merged).
...
- A banner appears on the merged post showing the former subject and who merged it.
- The duplicate thread disappears from the discussion feed.
- The author of the duplicate thread receives an in-app notification (and email) with a link to their post under the original thread.

<Note>
Only root-level threads that have not already been merged can be marked as duplicates. Replies cannot be merged using this feature.
</Note>
