...
Graders and instructors can merge a duplicate discussion thread into an existing thread. This is useful when students post the same question multiple times, or when a new post covers the same topic as an existing thread.
...
When a thread is marked as a duplicate:
...
Only root-level threads that have not already been marked as duplicates can be merged.
