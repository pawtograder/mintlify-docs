...
Graders and instructors can merge a duplicate discussion thread under another thread. This is useful when students post the same question multiple times or when a new post is clearly a duplicate of an existing one.

### How to mark a thread as a duplicate

1. Open the duplicate thread.
2. Click **Mark Duplicate** in the thread actions toolbar (visible to graders and instructors on root threads that have not already been marked as duplicates).
3. In the dialog, search for the original thread by title or post number (e.g. `#42` or `42`).
4. Select the original thread from the results list.
5. Click **Merge into original**.

### What happens when a thread is merged

- The duplicate thread and all of its replies become replies under the original thread.
- The duplicate thread displays a banner showing the former subject and who merged it.
- The duplicate thread disappears from the discussion feed — it is no longer listed as a root thread.
- The author of the duplicate thread receives an in-app notification (and email when the queue processor runs) with a deep link to their post under the canonical thread.

### Notes

- Only root threads can be marked as duplicates. Replies cannot be merged.
- A thread that has already been marked as a duplicate cannot be marked again.
- The merge cannot be undone through the UI.
