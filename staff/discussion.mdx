...
Graders and instructors can merge a duplicate root thread into the original thread. This is useful when students post the same question multiple times or when a thread is clearly a duplicate of an existing discussion.

### How to mark a thread as a duplicate

1. Navigate to the duplicate thread.
2. Click **Mark Duplicate** in the thread actions toolbar (visible to graders and instructors on root threads that haven't already been merged).
...
- All replies from the duplicate thread are moved under the original thread.
- A banner appears on the merged post showing the former subject and the name of the staff member who merged it.
- The duplicate thread is removed from the discussion feed.
- The duplicate's author receives an in-app notification (and email when the queue processor runs) with a link to their post in the canonical thread.

<Note>
Only root threads can be marked as duplicates. The action is not available on reply posts or threads that have already been merged.
</Note>
