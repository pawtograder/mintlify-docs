...
When a student posts a question that has already been answered in another thread, staff can merge the duplicate thread into the original. This keeps the discussion board tidy and ensures students find the canonical answer.

### How to mark a thread as a duplicate

1. Open the duplicate thread.
2. Click **Mark Duplicate** in the thread actions (visible to graders and instructors on root threads that have not already been merged).
3. In the dialog, search for the original thread by title or post number (e.g., `#42`).
4. Select the original thread from the results and click **Merge into original**.

After merging:

- The duplicate thread and all of its replies are moved under the original thread as replies.
- A banner appears on the merged post showing the former subject and the staff member who merged it.
- The duplicate thread no longer appears in the discussion feed.
- The author of the duplicate thread receives an in-app notification (and email when the queue processor runs) with a link to their post under the canonical thread.

<Note>
Only root-level threads can be marked as duplicates. Replies cannot be merged.
</Note>
