...
<Warning>
The **History** section is inert. The component is rendered without the course it
needs to run its query, so the query is never issued and the list is always
empty — on a course that has sent hundreds of messages as much as on a new one.

Past sends *are* stored in the database. They are simply not reachable from this
page, or anywhere else in the app. If you need a record of what you sent, Cc
yourself (accepting one copy per recipient) or keep your own draft.
</Warning>
