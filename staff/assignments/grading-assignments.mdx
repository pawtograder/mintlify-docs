...
When you release a review (individually, in bulk, or via **Release all**) and some of the selected reviews are not yet marked complete, Pawtograder shows a warning dialog before proceeding. The dialog tells you how many reviews are incomplete and gives you three options:

- **Cancel** — go back without releasing.
- **Review** — navigate to the assignment dashboard filtered to incomplete reviews so you can finish grading first.
- **Continue anyway** — release the incomplete reviews immediately.

Graders can still mark a review complete after it has been released (for example, to finish adding feedback). Graders cannot mark a released and already-completed review as incomplete; only instructors can do that.

<Tip>
Use the **Grading Complete** column filter on the submissions table to quickly find and address incomplete reviews before releasing grades.
</Tip>
