...
The assignment roster includes a **Placeholder** column (hidden by default — toggle it on from the column visibility controls) that distinguishes instructor-created stub submissions from real student work. A stub created via **Grade anyway** or automatically for a `no_submission` assignment shows **Yes**; a real student submission shows **No**. Use the Yes/No filter to quickly isolate or exclude placeholder rows when reviewing grades.
