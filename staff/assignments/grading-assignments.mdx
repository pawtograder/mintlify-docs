...
The assignment roster includes a hidden-by-default **Placeholder** column. A **Yes** badge (orange) indicates that the row's submission is an instructor-created stub — either from **Grade anyway** or from an auto-created stub on a no-submission assignment — rather than a real student submission. Use the column visibility toggle to show it, and the **Filter by placeholder** dropdown to isolate or exclude placeholder rows.
