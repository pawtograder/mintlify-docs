...
After a stub submission is graded, the assignment roster shows a **Placeholder** column (hidden by default — enable it via the column visibility controls). Rows where the submission was created by an instructor via **Grade anyway** show **Yes** with an orange badge; rows with a real student submission show **No**. You can filter the table by placeholder status to focus on or exclude instructor-created stubs.

<Note>
Assignments that don't expect a file submission (presentations, oral exams) also create placeholder stubs automatically — those rows are also flagged **Yes** in the Placeholder column.
</Note>
