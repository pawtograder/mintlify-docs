...
Instructors can release or unrelease grading reviews for selected submissions directly from the **Manage → Assignment** submissions table.
...
- **Select all in view** by clicking the **All in view** button in the column header — this selects every submission currently visible after applying any active filters.
- **Clear selection** by clicking **None**.

Row selection resets automatically when you change any column filter.

### Bulk release and unrelease

Once you have selected one or more submissions, the **Release** and **Unrelease** buttons at the top of the table become active and show the number of selected submissions (for example, **Release 5 selected submissions**). Click the button to release or unrelease the grading reviews for all selected submissions at once.

<Tip>
Use column filters (by grader, section, group mentor, etc.) before clicking **All in view** to release grades for a specific subset of students — for example, all submissions assigned to a particular grader or all groups with a specific mentor.
</Tip>

### Group mentor column

For group assignments where at least one group has an assigned mentor, a **Group mentor** column appears in the submissions table. This column:

- Shows the name of the mentor assigned to each group.
- Displays **—** for groups that have no mentor assigned.
- Supports multi-select filtering so you can filter the table to show only submissions for groups assigned to a specific mentor (or groups with no mentor).
- Can be toggled on or off using the column visibility controls.
