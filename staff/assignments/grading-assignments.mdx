...
The assignment roster includes a **Placeholder** column (hidden by default — toggle it on using the column visibility controls). A row shows **Yes** when the submission is an instructor-created stub (from **Grade anyway** or an auto-created stub on no-submission assignments) rather than a real student submission. Use the column filter to quickly separate placeholder grades from earned scores.
