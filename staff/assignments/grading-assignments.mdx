...
2. Click **View as this student** (orange button, visible to instructors only for active, non-disabled students).
3. A sticky orange banner appears at the top of every page: **"Viewing as [student name] — read only. You cannot make changes in this mode."**

While in student view:
- All write actions (submitting help requests, posting to discussion, liking/following, completing self-review, requesting regrades, etc.) are disabled.
- Unreleased grades are hidden, matching what the student actually sees.
- Instructor-only debug output and hidden autograder tests are not shown.
- The `/manage` section is inaccessible; navigating there redirects to the course home.

### Exiting student view

Click **Exit student view** in the orange banner to return to your normal instructor view. The view-as state persists across page loads until you exit.

<Note>
Student view is read-only and purely presentational. Your authentication and RLS identity are unchanged — any write attempted while in student view is rejected by the database's authorization rules.
</Note>
