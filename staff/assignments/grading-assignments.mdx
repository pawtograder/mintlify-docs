...
### Identifying placeholder submissions

The assignment roster includes a **Placeholder** column (hidden by default — toggle it on from the column visibility controls). A placeholder is a stub submission created by an instructor or grader via **Grade anyway**, or automatically for assignments that don't require a file submission (presentations, oral exams, etc.). The column shows **Yes** (orange badge) for placeholders and **No** for real student submissions.

Use the **Placeholder** filter in the column header to quickly isolate or exclude placeholder rows. This makes it easy to distinguish a student who earned a zero from one who simply hasn't submitted yet.
