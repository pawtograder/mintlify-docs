...
Students can now open a regrade request on a rubric check that was **not** applied to their submission — for example, if they believe a check should have been applied but wasn't. These "bare-check" requests appear in the rubric sidebar (for graders) and in the **Regrade Requests** panel on the student's **Grade** tab.

When resolving a bare-check request, the grader selects where to place the resulting grade (file + line for annotation checks, or artifact for artifact checks). Pawtograder then creates the backing grading comment automatically and recomputes the submission score.

If a grading comment that backs an open regrade request is deleted during re-grading, the request is automatically resolved with the reason **"grade changed"** and the student is notified. The student can still escalate the auto-resolved request to an instructor.
