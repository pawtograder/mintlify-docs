...
The **All regrade requests** dashboard gives instructors a single view of every regrade request across all assignments in the course. Access it from **Manage → All regrade requests** (or via the **All regrade requests** button in the Manage Assignments toolbar).

### What it shows

By default, the dashboard shows only active requests (status: **Pending** or **Escalated**). Draft and resolved requests are hidden to reduce noise. Toggle the **Hide draft and resolved** checkbox to see the full history.

Each row includes:
- **Status**: Pending, Escalated, Resolved, Closed, or Draft
- **Assignment**: Links to that assignment's regrade requests page
- **Rubric check**: The rubric item the student is contesting
- **Student / Group**: Who submitted the request, with an **Open** link to the submission files at the regrade request anchor
- **Assignee**: The grader responsible for the request
- **Points**: Initial, resolved, and escalation points
- **Escalation granted**: Whether the escalation changed the grade
- **Discord**: Link to the Discord thread for the request (if configured)
- **Created / Last updated**: Timestamps

### Filtering

Use the filter dropdowns to narrow by status, assignment, rubric check, student, assignee, or escalation outcome. Click **Clear Filters** to reset all filters and return to the default active-only view.
