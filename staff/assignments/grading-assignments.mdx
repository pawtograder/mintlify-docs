...
The assignment roster includes a **Placeholder** column (hidden by default — toggle it on using the column visibility controls) that distinguishes instructor-created stub submissions from real student submissions.

A row is marked **Yes** (orange badge) when the submission was created by an instructor or grader using **Grade anyway** or the bulk non-submitter stub flow — meaning no student-submitted work exists. Rows with a real submission show **No**. You can filter the roster by placeholder status using the column header dropdown.

This is useful when reviewing grading progress to quickly identify which "grades" are based on instructor-created stubs versus actual student work.
