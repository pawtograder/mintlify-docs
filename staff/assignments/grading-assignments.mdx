...
When a student has been graded via **Grade anyway** (or is on a no-submission assignment), their roster row shows a **Placeholder** badge in the **Placeholder** column. This distinguishes an instructor-created stub from a real student submission that earned a zero.

The **Placeholder** column is hidden by default. Toggle it on using the **Placeholder** checkbox in the column visibility controls above the roster table. You can also filter the roster to show only placeholder rows (or only non-placeholder rows) using the column header dropdown.
