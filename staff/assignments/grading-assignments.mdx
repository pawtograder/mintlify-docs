...
When an instructor or grader creates a stub submission for a student who hasn't submitted (via **Grade anyway** or the bulk non-submitter flow), the roster marks that row as a **Placeholder**. The **Placeholder** column is hidden by default — toggle it on from the column visibility controls. You can also filter the roster to show only placeholder rows (Yes) or only real submissions (No).

A placeholder is any submission created with `submitted_via = 'manual'`, which covers both **Grade anyway** stubs on regular assignments and the auto-created stubs on `no_submission` assignments (presentations, oral exams, etc.).
