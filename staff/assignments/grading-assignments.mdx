...
The **All regrade requests** page (accessible from **Manage → Assignments → All regrade requests**) gives instructors a single view of every regrade request across all assignments in the course.

By default, the dashboard shows only active requests (status: **Pending** or **Escalated**). Draft and resolved requests are hidden to reduce noise. Use the **Filter by Status** dropdown to include other statuses, or clear all filters to see the full history.
...
Each row includes an **Open** link that deep-links directly to the submission's files view with the regrade request highlighted. The assignment title links to that assignment's own regrade requests page.

<Tip>
Use the **All regrade requests** button on the Manage Assignments toolbar for quick access to this dashboard.
</Tip>
