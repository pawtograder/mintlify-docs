...
For row-selected bulk release, unrelease, mark-complete, and reassign
operations, plus filtering by group mentor, see
...
The assignment roster includes a **Placeholder** column (hidden by default) that distinguishes instructor-created stub submissions from real student work. Placeholder rows display an orange **Yes** badge; rows with a real submission show **No**.

A submission is marked as a placeholder when it was created by an instructor via **Grade anyway**, or when it was auto-created for a no-submission assignment (such as a presentation or oral exam). Placeholder status does not affect the grade — a non-zero score on a placeholder submission is recorded and displayed normally.

To show the column, check **Placeholder** in the column visibility controls above the roster table. You can also filter the roster to show only placeholder or only real submissions using the column header dropdown.
