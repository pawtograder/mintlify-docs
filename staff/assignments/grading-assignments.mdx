...
The assignment roster includes a hidden-by-default **Placeholder** column. A **Yes** badge (orange) indicates that the submission was created by an instructor or grader as a stub — either via **Grade anyway** or automatically for no-submission assignments — rather than submitted by the student. A **No** value indicates a real student submission.

Enable the column using the **Placeholder** checkbox in the column visibility controls. You can also filter the roster to show only placeholder or only real submissions using the column's Yes/No filter.

<Note>
If a student later makes a real submission, the stub is automatically deactivated and the Placeholder column updates to **No** for that student.
</Note>
