...
Every assignment is now **submission-optional** for grading purposes. Students who have not submitted appear on the grader roster with a **Not submitted** status. Instructors and graders can still grade them by clicking **Grade anyway** next to the student's name, which creates an empty stub submission so grading can proceed.

- If the student later makes a real submission, it automatically supersedes the stub and becomes the active submission.
- The assignment dashboard shows a **Not submitted: N** count alongside the graded/total progress so you can see at a glance how many students still need a submission.
- When using **Bulk Assign Grading**, enable the **Assign grading for all students** toggle to automatically create stub submissions for non-submitters and include them in the grading pool.
