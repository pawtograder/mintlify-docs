...
When you use **Grade anyway** on a non-submitter, or when an assignment is configured as `no_submission` (presentations, oral exams), Pawtograder creates a placeholder stub submission. The grader roster shows a **Placeholder** column so you can distinguish these instructor-created stubs from real student submissions.

- **Placeholder: Yes** — an instructor-created stub; no student-submitted work exists.
- **Placeholder: No** — a real submission from the student.

You can filter the roster by placeholder status using the column header dropdown, and toggle the column's visibility with the **Placeholder** checkbox in the column visibility controls.
