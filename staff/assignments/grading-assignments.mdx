...
1. Navigate to **Manage** → **Students** → select a student.
2. Click **View as this student** (orange button, visible to instructors only for active, non-dropped students).

A sticky orange banner appears at the top of every page while you are in student view. All write actions (submitting help requests, posting to the discussion board, liking posts, completing self-reviews, etc.) are disabled.

### What you see

- The student's assignment dashboard, including only grades that have been released to them.
- Autograder output and test results filtered to what the student can see (hidden tests and instructor-only debug output are not shown).
- The student's regrade requests.
- Discussion threads the student can access (instructor-only threads are hidden).
- The student's office hours queue position.

### Exiting student view

Click **Exit student view** in the orange banner. Your instructor view is restored immediately.

<Note>
Student view is purely presentational — your authentication and RLS identity are unchanged. Any write attempted while in student view is rejected by the database. The view-as state is stored in a per-course browser cookie and does not affect other users or sessions.
</Note>
