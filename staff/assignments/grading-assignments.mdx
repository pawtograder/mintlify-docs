...
### TA messaging when feedback is released

When a TA attempts to add or change marks on a submission whose review has already been released, Pawtograder now shows a clear warning instead of a generic permission error:
...
This warning appears in the rubric sidebar and as a toast notification when a TA tries to save a comment or annotation on a released review. The rubric criteria panel also shows a warning banner so TAs see the restriction before attempting to save.
...
## Bulk Releasing and Unreleasing Reviews

The instructor assignment submissions table supports **row selection** for bulk release and unrelease operations, giving you precise control over which submissions are released to students.

### How to bulk release or unrelease

1. Navigate to **Manage → Assignments → [Assignment]**.
2. Use the **checkbox column** on the left to select individual rows, or click **All in view** in the column header to select all submissions matching the current filters.
3. Click **None** to clear the selection.
4. Once rows are selected, the **Release** and **Unrelease** buttons become active and show the count of selected submissions (e.g. "Release 12 selected submissions").
5. Click the button to release or unrelease the selected submissions' reviews.

The selection is automatically cleared after a successful release or unrelease, and resets when you change column filters.

<Note>
Only submissions with an active submission (i.e. the student has submitted) can be selected. Rows without a submission are not selectable.
</Note>

### Group mentor column

When a group assignment has at least one group with an assigned mentor, a **Group mentor** column appears in the submissions table. You can:

- **Filter by mentor**: Use the multi-select dropdown under the column header to filter submissions by mentor name, or select **No mentor** to find groups without an assigned mentor.
- **Toggle visibility**: Use the column visibility controls to show or hide the Group mentor column.
...
Instructors can view the full commit history for any student or group repository directly from the assignment management interface. This is useful for investigating late submissions, verifying what code was pushed at a given time, or triggering grading for a specific commit.

### Viewing commit history

A **Commits** column appears in the assignment management table. Click the commit history icon for any student or group to open the commit history dialog, which shows:

- Commit SHA (linked to GitHub)
- Commit date and time
- Author
- Commit message
- Workflow status (e.g., "Recorded by webhook", "In progress", "Completed")
- All submissions associated with each commit

The dialog merges data from Pawtograder's database (commits received via push webhook) with the full commit list from the GitHub API, so you can see commits that were never submitted as well as those that were.

<Note>
Commit timestamps shown in the "Date" column are the author-supplied git dates, which can be backdated by adjusting the local clock. For commits received via push webhook, a "received" timestamp is also shown — this server-side timestamp is trustworthy. Commits sourced only from the GitHub API have no server-side receipt.
</Note>

### Triggering grading manually

From the commit history dialog on a student's submission page, instructors can manually trigger grading for any commit — including commits that were never submitted or that were pushed after the deadline.

To trigger grading for a specific commit:

1. Open the **Commit History** dialog from the submission view.
2. Find the commit you want to grade.
3. Click **Create submission and activate** (for commits with no existing submission) or **Activate** (for commits with an existing submission).
4. Confirm the action in the pop-up.

**Staff-triggered grading overrides deadlines and submission limits.** The resulting submission is automatically activated as the student's active submission once the grading workflow completes.

<Warning>
Manually triggering grading bypasses the assignment's due date and submission count limits. Use this only when you intentionally want to grade a commit outside the normal submission window.
</Warning>
...
When groups are configured after an assignment is released, some students may end up with both an individual repository and a group repository. The **Cleanup individual repositories** button on the Repositories page removes these mistaken individual repositories.

To clean up individual repositories:

1. Navigate to the assignment's **Repositories** page.
2. Click **Cleanup individual repositories** (disabled if no individual repos exist).
3. Review the warning — this will archive individual repos on GitHub and delete all related submissions, grades, and workflow data from Pawtograder.
4. Click **Continue**, then type `CLEANUP` to confirm.
5. Click **Confirm cleanup**.

The operation:
- Enqueues each individual repository for archival on GitHub (async)
- Deletes all submissions, grader results, regrade requests, and workflow data tied to those repositories
- Removes the repository records from Pawtograder
- Leaves group repositories untouched

<Warning>
This action cannot be undone. Only use it when you are certain that the individual repositories were created by mistake and should be replaced by group repositories.
</Warning>
