...
## Releasing and Unreleasing Submission Reviews

From the **Manage → Assignment** submissions table, instructors can release or unrelease grading reviews for selected submissions.

### Selecting submissions

The submissions table includes a checkbox column on the left. Use it to:

- **Select individual rows** by clicking the checkbox next to each submission.
- **Select all in the current filtered view** by clicking **All in view** in the column header.
- **Clear the selection** by clicking **None** in the column header.

Only submissions with an active submission can be selected. Changing a column filter automatically clears the current selection.

### Bulk release and unrelease

Once you have selected submissions, the **Release** and **Unrelease** buttons become active and show the number of selected submissions (for example, "Release 5 selected submissions"). Clicking either button applies the action only to the selected submissions and clears the selection when complete.

### Group mentor column

When a group assignment has at least one mentor assigned, a **Group mentor** column appears in the submissions table. The column shows the mentor's name for each group (or a dash for groups without a mentor) and includes a multi-select filter so you can filter the table by mentor — including a **No mentor** option to find unassigned groups. You can toggle the column's visibility using the column visibility controls above the table.
...
When a submission review has been released to students, TAs see a warning banner in the rubric sidebar explaining that marks are locked. Checkboxes, radio buttons, and comment forms are disabled for TAs on released reviews. If a TA attempts to save a comment or annotation and the action is blocked by the release lock, they see a clear message:
...
Instructors are not affected by this restriction and can edit any comment at any time.
