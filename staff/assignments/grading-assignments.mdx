...
The assignment roster includes a **Placeholder** column (hidden by default — toggle it via the column visibility controls) that distinguishes instructor-created stub submissions from real student work. A **Yes** badge in orange indicates a placeholder; **No** indicates a real submission. Use the Yes/No filter on the column to quickly find or exclude placeholder rows when reviewing grades.
