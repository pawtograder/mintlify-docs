...
Instructors can preview the course exactly as a specific student sees it — their dashboard, assignments, submissions, grades, discussion, and office hours — in a read-only mode.

To enter view-as mode:

1. Navigate to **Manage** → **Students** → select a student.
2. Click **View as this student** (available for active, non-disabled students).

While in view-as mode:
- A sticky orange banner at the top of every page indicates you are viewing as a student and provides an **Exit student view** button.
- All write actions (submitting, posting, liking, requesting regrades, etc.) are disabled.
- Unreleased grades are hidden, matching what the student actually sees.
- Staff-only content (instructor-only discussion threads, hidden test output, debug build logs) is not shown.
- The `/manage` pages redirect back to the course home.

View-as mode uses a per-course cookie and does not change your authentication or RLS identity — it is purely a presentation layer.
