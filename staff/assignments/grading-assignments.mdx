...
The assignment roster includes a **Placeholder** column (hidden by default) that distinguishes instructor-created stub submissions from real student work. Stubs created via **Grade anyway**, or auto-generated for no-submission assignments (presentations, oral exams), show **Yes** with an orange badge. All other submissions show **No**.

To show the column, check **Placeholder** in the column visibility controls above the roster table. You can also filter the roster to show only placeholder or only real submissions using the Yes/No filter on the column header.
