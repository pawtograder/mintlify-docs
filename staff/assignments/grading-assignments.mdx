...
The **Manage → Assignment** submissions table gives instructors a full view of all student submissions for an assignment. It supports filtering, sorting, and bulk actions.

### Group mentor column

For group assignments where at least one group has an assigned mentor, a **Group mentor** column appears automatically in the table. You can:

- Filter submissions by mentor using the multi-select filter (includes a **No mentor** option for groups without an assigned mentor)
- Toggle the column on or off using the column visibility controls

### Bulk release and unrelease

You can release or unrelease grading reviews for a selected subset of submissions rather than all submissions at once:

1. Use the checkbox column on the left to select individual rows, or click **All in view** in the column header to select every submission matching the current filters.
2. Click **Release N selected submissions** or **Unrelease N selected submissions**. The button label updates to show the number of currently selected submissions.
3. The action applies only to the selected submissions. After the action completes, the selection is cleared automatically.

The release and unrelease buttons are disabled when no submissions are selected.
