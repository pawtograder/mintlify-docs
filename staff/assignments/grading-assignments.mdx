...
- **Assign to lab leaders**: Each submission is automatically assigned to all lab leaders of the student's [lab section](/staff/lab-sections). No manual grader selection is needed.
...
To select assignees (for "By submission" and "By rubric part" modes), you can filter by role (**Instructors**, **Graders**, or **Instructors and graders**), and optionally filter those staff by tag. The form will show which graders will be assigned, and you can manually override the selection of graders.
...
Each review assignment has its own due date — separate from the
student's submission due date. Adjust them per row by editing the
review assignment in the Grading Assignments table, or in bulk via the
**Bulk edit grading due dates** / **Bulk edit self-review due dates**
buttons; details in
[Bulk Actions → Bulk Review Due Date Updates](/staff/assignments/bulk-actions#bulk-review-due-date-updates).
...
Individual grading is not an option on the grading-assignment flow. It is a property of a **rubric part**, set in the rubric editor or in rubric YAML:

- **Individual grading** (`is_individual_grading`) grades the part once per group member. The rubric sidebar shows a separate section per student, and checks and comments are scoped to that student.
- **Assign to student** (`is_assign_to_student`) lets the grader pick which single student the part applies to, or skip it.

A part can use one mode or the other, not both. See [Rubrics](/staff/assignments/rubrics) for the part-level fields and [Handgrading](/staff/assignments/handgrading) for what graders see.
...
### Reassigning a single review

Click the edit icon on the review's row in the **All Assignments** table to change its assignee or due date. See [Editing individual review assignments](#editing-individual-review-assignments) below.

### Reassigning one grader's remaining work

Reassignment in bulk is grader-centric, not submission-centric. Follow the **Reassign Grading by Grader** link on the **Manage Review Assignments** page, then:

1. Select the grader whose remaining work you want to reassign.
2. Choose the rubric.
3. Set how many of that grader's incomplete submissions to reassign. The form tells you how many are incomplete for this rubric.
4. Choose the target role (**Instructors**, **Graders**, or **Instructors and graders**), optionally filter those staff by tag, and optionally name specific assignees.
5. Choose a load-balancing strategy: **Balance based on new assignments** or **Balance based on all assignments**.
6. Set the review due date.
7. Click **Prepare Reassignments**, adjust the draft in the drag-and-drop preview, then click **Reassign**.

Each new grader inherits the same rubric parts the original grader was assigned.

### How reassignment works

- When a submission is reassigned, it appears in the new grader's pending work list immediately.
- Even if the new grader had already marked a different rubric part of the same submission complete, the reassigned part still arrives as pending rather than being treated as already done.
...
- Graders are not notified automatically. Use **Generate Reminder Email** on the Grading Progress Dashboard to tell them.
...
For row-selected bulk release and unrelease, the assignment-wide
**Finalize grading** action, and filtering by group mentor, see
...
Use the [Grading Status](/staff/assignments/grading-status) view to
monitor overall progress, per-grader workload, and pending reviews
...
Some assignments are configured with `repo_mode: no_submission` — they are graded manually with no file upload or autograder output. For these assignments:

- The **Files** and **Autograder Detail** tabs are hidden on the submission page. Only the **Grade** tab is shown.
- The submission page opens directly on the grading view, with the full editable rubric rendered at full width.
- Stale links that point at `/files` (such as "next incomplete review" navigation) still work — they redirect to the grading view automatically.
- The **Survey** tab still appears if a survey is linked to the assignment.

This applies to both the student-facing submission route and the staff grading route.

## Grading Students Without a Submission

Every enrolled student appears on the assignment roster, even if they haven't submitted yet. Students without an active submission show a **"Not submitted"** status and a **Grade anyway** button.

### Grade anyway

Clicking **Grade anyway** on a non-submitter row opens a confirmation dialog, then creates an empty stub submission for that student (or their group, for group assignments) and navigates directly to the grading page. The student can see the stub submission once it is created.

<Note>
If the student later makes a real submission, it automatically supersedes the stub — the stub is deactivated and the roster updates to point at the real submission.
</Note>

### Grading progress counts

The grading progress panel counts only students with an active submission toward the totals. A separate **Not submitted: N** indicator shows how many enrolled students still have no submission for the assignment.

### Bulk assign for all students

When using **Bulk Assign Grading**, the default behavior assigns grading only to students who have already submitted. Enable the **Assign grading for all students** toggle to also create empty stub submissions for non-submitters (respecting the current section, lab, and tag filters) before drafting grading assignments. This is useful when you want to ensure every enrolled student receives a grading assignment regardless of submission status.
...
- **[Lab section](/staff/lab-sections)**: Shows the lab section name, meeting times, location, and assigned section leaders.
