...
The assignment roster includes a hidden-by-default **Placeholder** column that identifies stub submissions created by instructors or graders (via **Grade anyway** or auto-created for `no_submission` assignments). Rows with a placeholder submission show an orange **Yes** badge; rows with a real student submission show **No**.
...
<Note>
A placeholder submission is not the same as a zero score. A student who submitted and earned zero points will show **No** in the Placeholder column; a student graded via **Grade anyway** will show **Yes**.
</Note>
