...
The **All regrade requests** dashboard gives instructors a single view of every regrade request across all assignments in the course. Access it from **Manage → Assignments → All regrade requests** (or via the toolbar button on the Manage Assignments page).

### Default view

By default, the dashboard shows only **opened** and **escalated** requests — the active requests that need attention. Draft and resolved requests are hidden to reduce noise.

### Filtering

Use the filter controls to narrow the list by:

- **Status**: Draft, Pending (opened), Resolved, Escalated, or Closed
...
Click **Clear Filters** to reset all filters and return to the default view.

### Columns

Each row shows:

- **Status**: Current state of the request (Pending, Escalated, Resolved, Closed, Draft)
- **Assignment**: The assignment the request is for (links to that assignment's regrade requests page)
- **Rubric check**: The rubric check the request concerns
- **Student/Group**: The student or group who submitted the request
- **Assignee**: The grader assigned to handle the request
- **Initial/Resolved/Escalation points**: Point values at each stage
...
Click **Open** next to any row to jump directly to the submission's file view with the regrade request highlighted.
