...
### Confirm review button and auto-advance

A **Complete Review** button now appears at the bottom of the rubric sidebar, so graders no longer need to scroll back to the top after finishing all their checks. When all required checks are satisfied, the button expands to show two options:

- **Complete** — marks the review complete and stays on the current submission.
- **Complete + Next** — marks the review complete and immediately navigates to the next incomplete submission in your queue.

The "Complete + Next" button only appears when there is another incomplete submission to advance to. It wraps around: if there are no incomplete submissions after the current one, it jumps to the first incomplete submission overall.
...
Students can now request a regrade for a rubric check that was **not applied** to their submission — for example, if they believe a check should have been applied but wasn't. These "bare-check" regrade requests work the same way as comment-backed requests:

1. The student opens a regrade request from the **Grade** tab on their submission page.
2. The request appears in the grader's queue with the check name and "Not applied" context.
3. When the grader resolves the request, Pawtograder automatically creates the backing grading comment and updates the submission score.

**For graders resolving bare-check requests:** the resolve popover asks you to specify where the grade should appear (for annotation checks, you must select a file and line number or artifact). The submission score updates automatically once you confirm.

### Auto-resolution of dangling regrade requests

When a grader deletes or re-grades a comment that has an open regrade request attached to it, the request is automatically resolved with status "Resolved" and reason "comment_deleted". The student receives a notification explaining that the grade was changed during re-grading and they can escalate if they still disagree.

This prevents regrade requests from getting stuck in a "Pending" state when the underlying grade has already changed. Auto-resolved requests are **not** terminal — students can still escalate to an instructor.
...
**Comment editing after review completion:** Graders can continue to insert and update comments after completing a review assignment, as long as the review has not been released to students. Once a review is released, only instructors can edit comments. This allows graders to refine feedback before the final release while preventing changes to student-visible reviews.

After completing a review, graders can still edit their comments and rubric checks. However, once a review is released to students, only instructors can make further edits (graders lose edit access to released reviews).

After completing a review, graders can still edit their comments and annotations. However, once a review is released to students, only instructors can make further edits (graders lose edit access to released reviews).

Graders can continue to edit their comments and rubric feedback even after marking a review as complete, as long as the review has not been released to the student. Once a review is released, only instructors can make further edits to protect the integrity of feedback that students have already seen.
