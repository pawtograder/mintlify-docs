...
If you want students to have a GitHub repository but do not need automated grading, you can create a **repo-only** assignment by unchecking **Enable autograder (GitHub Actions)** when creating the assignment. In this mode:

- The handout and student repositories are created without `grade.yml`, so no GitHub Actions run and students never see a failing check.
- Every push to the student repository creates a submission for you to grade by hand — no `#submit` tag is needed.
- A solution repository is still created for your reference solution and grading notes.
- Empty submission detection still works: pushes that are identical to the handout starter are flagged.

You can toggle the autograder on or off after creation from the **Autograder** page. See [Assignment Configuration](/staff/assignments/configuration#autograder) for details.
