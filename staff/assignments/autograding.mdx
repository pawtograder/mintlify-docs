...
<Note>
If you want a GitHub repository for students but do not need automated grading, you can create a **repo-only assignment** by unchecking **Enable autograder (GitHub Actions)** when creating the assignment. The handout is created without `grade.yml`, so no Actions run and every push becomes a submission for hand-grading — no `#submit` tag required. You can toggle the autograder on or off later from the assignment's **Autograder** page. See [Assignment Configuration](/staff/assignments/configuration) for details.
</Note>
