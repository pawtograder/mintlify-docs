...
- `rate_limit` is optional, and so is each field inside it. A field you omit, or set to `0`, is not enforced at all: there are no server-side defaults behind these numbers. `assignment_total` and `class_total` are lifetime counts of Feedbot responses for the assignment and for the class, with no time window.
...
<Warning>
`cooldown` is in **minutes**, not seconds, and it is assignment-wide rather than per-student. The server measures the elapsed time since the most recent Feedbot request on the assignment by any student, and until that window passes every student gets "Rate limit: Please wait N more minute(s) before requesting Feedbot feedback for this assignment." The action also sends `cooldown: 5` whenever Feedbot is enabled, even if you omit `rate_limit` entirely, so a five-minute assignment-wide cooldown is what you get by doing nothing.
</Warning>
