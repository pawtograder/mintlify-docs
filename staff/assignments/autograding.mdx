...
The in-app config editor resolves this modeline locally (no network request needed) for v4 schemas, so validation and autocomplete work even in restricted environments.
</Note>
...
`pawtograder.yml` lives at the root of the grader/solution repo. The top-level
`grader: overlay` field is **required** (as of v4 of the schema). It has three
...
<Note>
The `warn` policy was removed in v4 of the schema. If you have an existing `pawtograder.yml` using `policy: warn`, update it to either `fail` or `ignore`.
</Note>
