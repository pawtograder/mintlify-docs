...
<Note>
The autograder is optional. If you want a repository-based assignment without
GitHub Actions grading — for example, a project you grade entirely by hand —
uncheck **Enable autograder (GitHub Actions)** when creating the assignment.
The handout is created without `grade.yml`, so no Actions run and every push
creates a submission for hand-grading. See
[Assignment Configuration](/staff/assignments/configuration) for details.
</Note>
