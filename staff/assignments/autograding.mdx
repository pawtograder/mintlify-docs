...
Pawtograder compares the files it collects from a submission against the
starter code in the handout, and by default it **rejects** a submission
whose collected files are identical. Nothing in the assignment editor
turns that rejection off, so it is the behavior on every assignment.

What the student sees depends on how the submission arrived:

- On the GitHub Actions path, the submission is deleted and the run fails with "Empty submissions are not permitted for this assignment. Please commit your changes before submitting." Staff-triggered submissions are exempt from the check.
- On the push-direct path, an inactive, fileless submission is kept on purpose and carries a student-visible error explaining that the commit was not recorded because it is identical to the starter code. The assignment page labels that submission "Error".

Empty submission detection looks only at the files that match
`submissionFiles`. An assignment that declares no `submissionFiles` has no
comparable handout hash, so the check is skipped and every push is
accepted; repo-only assignments never get empty detection.
