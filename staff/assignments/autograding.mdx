...
You can create a repository-backed assignment without an autograder. In this mode:
...
- **Every push** to the student repository creates a submission for you to grade by hand — no `#submit` tag needed.
- A solution repository is still created for your reference solution and grading notes.

To create a repo-only assignment, uncheck **Enable autograder (GitHub Actions)** in the **Autograder** section of the assignment creation form. You can also toggle this on or off later from the assignment's **Autograder** configuration page.

<Note>
When you disable the autograder on an existing assignment, the grading workflow is removed from the handout repository and existing student repositories are queued to sync the change. Until a student repo syncs, it still carries the old workflow — pushes to it will show a failing check until the sync completes.
</Note>

<Warning>
If the assignment forks from a prior assignment (`fork_from_prior_assignment` mode), both assignments share the same handout repository and must have the same autograder setting. You cannot disable the autograder on one without the other.
</Warning>
