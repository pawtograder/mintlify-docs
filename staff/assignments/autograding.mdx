...
that runs inside each student repository on every push.

<Note>
If you want a GitHub repository for students without running an autograder — for example, a writing assignment or a project where you grade by hand — uncheck **Enable autograder (GitHub Actions)** when creating the assignment. The handout is created without `grade.yml`, so no Actions workflow runs and every push creates a submission for hand-grading automatically. See [Assignment Configuration](/staff/assignments/configuration) for details.
</Note> It overlays the
