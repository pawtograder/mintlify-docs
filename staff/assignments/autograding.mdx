...
<CardGroup cols={2}>
  ...
  <Card title="Submission Limits" icon="gauge" href="#submission-limits">
  ...
  grade:
    ...
    steps:
      ...
      - name: Collect Submission and Run Grader
        uses: pawtograder/assignment-action@v4
        with:
...
<Note>
**Runner label and action version.** `runs-on: ubuntu-latest` and
...
are live and maintained, so a course template pinned at `@v3` is current, not stale.

A self-hosting deployment normally substitutes its own self-hosted runner label here. The
...
Larger classes limit concurrency, so pick the smallest one that can run your build.

Follow whatever your own course template pins for both the runner label and the action
version. Do not edit a working template to match the example above.
</Note>
...
The `on:` block above is what decides when a student submits. It is the workflow's own configuration, not something Pawtograder imposes, so editing it changes the submission rule for the assignment.
...
<Warning>
Keep `workflow_dispatch:` no matter what else you change. It is how Pawtograder starts a run on demand, and removing it breaks both the `#submit` path below and the per-commit trigger button in the Commit History dialog.
</Warning>
...
Assignments configured without an autograder have no `grade.yml` at all. There, every push to the default branch is collected as a submission for hand grading, and neither trigger applies. Pushes to other branches are recorded in the commit history but are not submissions. See [Configuration → Autograder](/staff/assignments/configuration#autograder).
...
[Workflow Runs → Grading Errors](/staff/workflow-runs#the-grading-errors-tab). The handout template's own opening comment warns students that tampering could expose the instructor solution and that instructors will be notified.
...
<Warning>
The hash is tied to a specific handout repository. If you re-point an assignment at a different handout, Pawtograder has to re-derive the hash — and if that step fails, the save succeeds while every student submission starts failing with a workflow sha mismatch. The toast says so and tells you the fix: **save this assignment again to retry**. If students report the mismatch error right after you changed a handout, that is the cause.
</Warning>
...
On the **Configure Autograder** page, not on
[Edit Assignment](/staff/assignments/configuration):

- **Maximum number of submissions per student (count)** — a whole number, or blank for no limit.
...
Leaving **either** field empty means no limit at all — so choosing *No limit* in the period dropdown removes the limit even if a count is still filled in. Clear the count field rather than typing `0`: a `0` also disables enforcement, but it reads as a deliberate zero to the next person who opens the page.
...
- `#NOT-GRADED` submissions **do** count. There is no exclusion for them.
- On a group assignment the quota is **shared by the whole group**.
- Regrade re-runs and staff-triggered submissions are exempt.

### The count students see can disagree with the limit

The remaining-submissions count on the student's assignment page is calculated
separately from the limit that actually blocks a push. The two normally agree, but on
group assignments they can drift apart, so treat the student-facing number as
approximate when you are fielding a question about it.

### What the student sees

Two different strings, for the same setting:

- The GitHub Actions run fails with `Submission limit reached (max N submissions per 1 day). Please wait until MM/dd/yyyy HH:mm to submit again.`
- The banner on the assignment page describes the same 86400 seconds as "per 24 hours", and adds that submissions scoring zero do not count.

<Warning>
Two things to know before you rely on the banner:

- It is only rendered when the assignment has an autograder, which is correct — submission limits throttle autograder runs.
- It **disappears entirely for any period under an hour**, because the period is converted to whole hours and 10 minutes rounds to zero. The limit is still enforced. If you pick the *10 minutes* option, students get no on-page warning at all and only find out when a push fails, so say so in your own course materials.
</Warning>

A throttled push also writes a public error row visible under
[Workflow Runs → Grading Errors](/staff/workflow-runs#the-grading-errors-tab). It has no submission attached, so it does not appear on any student submission page.
...
- `grader` — the grader type. `overlay` is the only accepted value.
...
- `policy: fail` — if the linter finds errors, the rest of grading is skipped and the student receives a zero. A finished run that scored zero does not count against the assignment's [submission limit](#submission-limits) — and every assignment has one, so this exemption matters.
...
<Warning>
`allow_partial_credit` defaults to `false`, so unless you set it a unit is all-or-nothing: a student who passes four of five matched tests earns zero for that unit, not 80%.
</Warning>
...
- `locations` is an array of strings. A class name counts every mutant whose location starts with that class. Any entry that does not match a class is then matched as a substring against each mutant's name, which under `java-gradle` is the Pitest mutator name, so `MathMutator` selects every math mutant regardless of class.
- Scoring uses **either** `breakPoints` **or** `linearScoring`, not both:
  - `breakPoints` — array of `{ minimumMutantsDetected, pointsToAward }`, ordered from the **highest** `minimumMutantsDetected` to the lowest. The unit awards the points of the first entry in list order whose threshold the student met, and it reports the unit's maximum score as the first entry's `pointsToAward`.
  - `linearScoring: { total_faults, points }` — awards `(detected / total_faults) * points`, rounded to two decimal places.
- `hideFeedbot: true` — same meaning as on regular units.

<Warning>
List `breakPoints` in descending order. Order is not sorted for you: with `minimumMutantsDetected: 5` listed before `minimumMutantsDetected: 10`, a student who detects 10 mutants is awarded the 5-mutant entry's points, and the unit's maximum score is reported as that entry's points too.
</Warning>

<Note>
Line-range locations (`ClassName-startLine-endLine`) only match mutants whose reported location has the form `Class:startLine:endLine`. The `java-gradle` preset reports Pitest locations as `Class:lineNumber`, with no end line, so a line range never matches anything under Pitest. Restrict a mutation unit to part of a class only if you supply your own `python-script` mutation runner that emits three-part locations; otherwise use whole-class locations.
</Note>
...
<Warning>
`mutantAdvice` only fires when your mutation runner names each mutant `"<sourceClass> <targetClass>"`. The `java-gradle` preset names mutants after the Pitest mutator alone, which contains no space, so the action never derives a `targetClass` and no advice is ever shown. Mutant hints therefore require a `python-script` mutation runner.
</Warning>
...
feedbot:
  ...
  rate_limit:
    cooldown: 5               # minutes since the last request on this assignment
    assignment_total: 100     # cap per assignment (omit for no cap)
    class_total: 5000         # cap per class (omit for no cap)
```
...
- `rate_limit` is optional, and so is each field inside it, but the fields do not all default the same way. `assignment_total` and `class_total` are lifetime counts of Feedbot responses for the assignment and for the class, with no time window, and each is enforced only if you give it a number: omit one, or set it to `0`, and that cap does not apply. `cooldown` is the exception — omit it and you still get a five-minute cooldown, and `cooldown: 0` is the only way to run with none.
...
<Warning>
`cooldown` is in **minutes**, not seconds, and it is assignment-wide rather than per-student. The server measures the elapsed time since the most recent Feedbot request on the assignment by any student, and until that window passes every student gets "Rate limit: Please wait N more minute(s) before requesting Feedbot feedback for this assignment." The five-minute cooldown is what you get by doing nothing: the action fills it in whenever Feedbot is enabled, even if you omit `rate_limit` entirely. Write `cooldown: 0` if you want students to be able to ask back to back.
</Warning>
...
Pawtograder can compare the files it collects from a submission against
the starter code in the handout, and where it can, it **rejects** a
submission whose collected files are identical. Two separate things decide
what happens: whether the assignment gives the check anything to compare,
and how the submission arrived.

### When the check applies

The comparison covers exactly the files matching `submissionFiles` in the
autograder configuration, so:

- **An assignment that declares `submissionFiles`** has empty submission detection in force. Nothing in the assignment editor turns that rejection off.
- **An assignment that declares no `submissionFiles`** gives the check nothing to compare, so it does not apply and every push is accepted. Repo-only assignments carry no autograder configuration and therefore never see empty submission detection.

Because the check follows `submissionFiles`, it also respects whatever
...
On an assignment where the check applies, what the student sees depends on
how the submission arrived:

- **Submissions graded by GitHub Actions.** The submission is discarded and the run fails with "Empty submissions are not permitted for this assignment. Please commit your changes before submitting."
...
Either way the submission cannot be graded, and the student has to change
a submitted file and push again. Staff-triggered submissions are exempt
from the check, so you can still submit an unchanged repository while
...
See [Workflow Runs](/staff/workflow-runs) for the status of the run a rerun
dispatches.
...
The action submits the tests, lint output, and logs back to the grading server. If the grader emitted any `artifacts`, they are uploaded to Pawtograder's file storage via the signed URLs returned by the server. A summary table is also written to the GitHub Actions job summary.
</Step>
</Steps>
