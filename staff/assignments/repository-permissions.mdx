...
When a student repository cannot be created (for example, because the template repository is empty or missing), Pawtograder records the failure reason on the repository row instead of silently retrying forever. The **Repositories** page shows a **Creation failed** badge with the error message and a **Retry creation** button.

### Retrying a failed repository

1. Fix the underlying issue (for example, push at least one commit to the template repository).
2. Navigate to the assignment's **Repositories** page.
3. Find the row showing **Creation failed** and click **Retry creation**.
4. The system clears the recorded error and re-enqueues the creation job. The page updates automatically when the repository becomes ready.

<Note>
Pawtograder also runs a background reconciler every 15 minutes that automatically re-enqueues repositories that are stuck in a pending state (no recorded error). Repositories with a recorded creation error are left for an instructor to retry after the root cause is fixed.
</Note>
