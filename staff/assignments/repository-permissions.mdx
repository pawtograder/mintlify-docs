...
If a repository could not be created (for example, because the template repository is empty or missing), the **Repositories** page shows a **Creation failed** status with a human-readable reason and a **Retry creation** button.

### Retrying a failed repository

1. Resolve the underlying issue (for example, push at least one commit to the template repository).
...
<Note>
Pawtograder also runs an automatic background reconciler every 15 minutes that re-enqueues repositories stuck in a pending state (no recorded error). Repositories with a recorded creation error are left for an instructor to retry after fixing the root cause.
</Note>
