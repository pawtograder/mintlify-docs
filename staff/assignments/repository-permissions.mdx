...
When a student repository cannot be created (for example, because the template or source repository is empty or missing), Pawtograder records the failure reason on the repository row instead of retrying indefinitely. The **Repositories** page shows a **Creation failed** badge with the error message.

### Retrying a failed repository

1. Fix the underlying problem (for example, push at least one commit to the template repository).
2. Navigate to the assignment's **Repositories** page.
3. Find the affected row and click **Retry creation**.
4. The system clears the recorded error and re-enqueues the creation job.

<Note>
Pawtograder also runs a background reconciler every 15 minutes that automatically re-enqueues repositories that are stuck in a pending state (no recorded error, not yet ready). Repositories with a recorded creation error are left for an instructor to retry after the underlying issue is resolved.
</Note>
