...
If a student's repository could not be created (for example, because the template repository is empty or missing), the **Repositories** page shows a **Creation failed** status with a human-readable reason and a **Retry creation** button.
...
4. The system re-queues the creation job. The page updates automatically once the repository is ready.

<Note>
Pawtograder also runs an automatic background reconciler every 15 minutes that re-enqueues repositories stuck in a pending state (no recorded error). Repositories with a recorded creation error are left for an instructor to retry after the root cause is fixed.
</Note>
