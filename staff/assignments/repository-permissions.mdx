...
When groups are configured after an assignment is released, some students may end up with both an individual repository and a group repository. Use the **Cleanup individual repositories** button to remove the mistaken individual repos.
...
After the cleanup completes, the repository table refreshes automatically and a summary toast shows how many repositories were queued for archival.

<Warning>
This action cannot be undone. Only run it when you are certain the individual repositories are no longer needed.
</Warning>
