...
<Note>
When dissolving an empty group (a group with no members), Pawtograder
preserves the group's GitHub repository if it already has submissions
associated with it. This prevents accidental data loss when cleaning up
groups that were created but never used for active work.
</Note>
