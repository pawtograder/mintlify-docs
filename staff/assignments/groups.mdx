...
- **Copy from a prior assignment** — reuse the group composition (and
  mentor assignments) from a previous assignment using the **Copy Groups from Prior Assignment** button on the Groups page.
...
To reuse group compositions from a previous assignment, click **Copy Groups from Prior Assignment** at the top of the Groups page. This opens a dialog where you select the source assignment.
...
- Overwrites all current group memberships for the target assignment with the groups and members from the source assignment.
- Preserves group names and mentor assignments from the source.
- Removes existing target memberships and deactivates any active submissions for removed members.
- Deletes empty target groups that have no submission history; groups with prior submissions are preserved so graded work stays intact.
- Triggers GitHub repository creation for any new groups if the assignment has been released.

Only prior assignments with group configuration and at least one group are shown as source options. The dropdown lists them in due-date order.
