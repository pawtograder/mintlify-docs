...
- **Copy from another assignment** — reuse the group composition (and
  optionally mentor assignments) from a previous assignment (see below).
...
To reuse group compositions from an earlier assignment, click **Copy Groups from Prior Assignment** at the top of the Groups page. This opens a dialog where you select the source assignment.

<Warning>
This action runs immediately — it does not wait for the page's **Publish Changes** button. Clicking **Overwrite Groups Now** applies the change at once.
</Warning>

What happens when you overwrite:

- All current group memberships on the target assignment are removed.
- Groups and memberships from the source assignment are copied over, matched by group name.
- Existing groups that become empty and have no submission history are deleted; groups with submission history are preserved so graded work stays intact.
- Any submissions belonging to removed members are deactivated.
- If the target assignment has a template repository and has already been released, new group repositories are provisioned automatically.

Only assignments that are earlier (by due date) and already have groups with members appear in the source dropdown.
