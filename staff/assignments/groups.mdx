...
- **Copy from a prior assignment** — reuse the group composition (and
  ...
  **Copy Groups from Prior Assignment** button on the Groups page.
...
The **Copy Groups from Prior Assignment** button (top of the Groups page) lets you overwrite the current assignment's group memberships with the groups and members from a prior assignment.

<Warning>
This action runs immediately when you click **Overwrite Groups Now** — it does not wait for the page's **Publish Changes** button. It cannot be undone.
</Warning>

When you copy groups:

1. Click **Copy Groups from Prior Assignment**.
2. Select the source assignment from the dropdown (only group assignments with existing members are shown, sorted by due date).
3. Review the warning that this is an immediate, non-staged action.
4. Click **Overwrite Groups Now**.

After the copy:

- Source group names and mentor assignments are applied to the target assignment.
- All existing target memberships are removed and replaced with the source memberships.
- Submissions belonging to removed members are deactivated.
- Empty target groups that have no submission history are deleted (and their GitHub repos are archived).
- Empty target groups that *do* have submission history are preserved so graded work stays intact.
- If the assignment is released and has a template repo, GitHub repos are provisioned for any new groups.
