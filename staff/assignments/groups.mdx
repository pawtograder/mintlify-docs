...
- **Copy from another assignment** — reuse the group composition (and mentor assignments) from a previous assignment using the **Copy Groups from Prior Assignment** button on the Groups page.
...
To copy group membership from a previous assignment, click **Copy Groups from Prior Assignment** at the top of the Groups page. Select the source assignment from the dropdown (only group assignments with existing groups that were due before this assignment are shown) and click **Overwrite Groups Now**.

<Warning>
This action runs immediately and does not wait for the page's **Publish Changes** button. It overwrites all current group assignments for this assignment with the groups and memberships from the selected source assignment. Existing groups that become empty and have no submission history are removed; groups with submission history are preserved.
</Warning>

After the copy completes, a success toast shows how many groups were copied, members assigned, previous memberships removed, and empty groups deleted or preserved.
