...
- **Copy from a prior assignment** — overwrite the current group
  composition with groups and memberships from a previous assignment
  (see below).
...
Use the **Copy Groups from Prior Assignment** button at the top of the Groups page to overwrite this assignment's group memberships with the groups and members from an earlier assignment.
...
The dialog shows a dropdown of eligible source assignments (group assignments with at least one group, due before this assignment). After selecting a source and confirming:

- Source group names and mentor assignments are copied to this assignment. Existing target groups with the same name are reused so repository and grading history stays intact.
- All existing target memberships are removed and replaced with the source memberships.
- Submissions for removed members are deactivated.
