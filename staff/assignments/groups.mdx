...
- **Copy from a prior assignment** — reuse the group composition (and
  mentor assignments) from a previous assignment using the
  **Copy Groups from Prior Assignment** dialog on the Groups page.
...
The **Copy Groups from Prior Assignment** button appears at the top of
the groups table for any group-configured assignment. Use it to
overwrite the current assignment's group memberships with the groups
and members from an earlier assignment.

<Warning>
This action runs immediately — it does **not** wait for the page's
**Publish Changes** button. Clicking **Overwrite Groups Now** applies
the change at once.
</Warning>
...
2. Click **Copy Groups from Prior Assignment**.
3. Select the source assignment from the dropdown. Only prior
   assignments (due before this one) that have groups with members are
   listed, sorted by due date.
4. Review the warning that this action is immediate and will overwrite
   existing memberships.
5. Click **Overwrite Groups Now**.

After the copy completes, a success toast shows how many groups were
copied, members assigned, previous memberships removed, and empty
groups deleted or preserved.

### What the copy does

- **Upserts target groups** — groups from the source assignment are
  created in the target assignment (matched by name). Existing
  target groups with the same name are reused so repository and
  grading history links survive.
- **Replaces all memberships** — every existing membership in the
  target assignment is removed and replaced with the source
  assignment's memberships. Submissions for removed members are
  deactivated.
- **Cleans up empty groups** — target groups that end up empty and
  have no submission history are deleted (and their GitHub
  repositories archived). Groups with submission history are
  preserved so graded work stays intact.
- **Provisions repositories** — if the target assignment has a
  template repo configured and has already been released, new
  repositories are created for any groups that don't have one yet.
