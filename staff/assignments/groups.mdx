...
The button is disabled until the course has another assignment whose
submission type allows groups. On a course with only one group assignment
there is no source to copy from, so the control appears greyed out rather
than missing.
