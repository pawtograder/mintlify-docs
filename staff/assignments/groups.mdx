...
When a student is moved out of a group and that group becomes empty,
Pawtograder dissolves the empty group and archives its GitHub
repository. However, if the group's repository already has submissions
(for example, a student who submitted individually before being moved
into a real group), the group and its repository are **preserved**
intact — the submission history and graded work are not deleted. The
preserved group is excluded from the permission re-sync so its GitHub
