...
- **Copy from a prior assignment** — overwrite the current group composition with groups and memberships from a previous assignment (see below).
...
The **Copy Groups from Prior Assignment** button at the top of the Groups page lets you overwrite the current assignment's group memberships with the groups and members from an earlier assignment.
...
1. Click **Copy Groups from Prior Assignment** on the Groups page.
2. Select the source assignment from the dropdown. Only group assignments with existing members that have an earlier due date are listed.
3. Review the warning that this is an immediate, non-staged action.
4. Click **Overwrite Groups Now**.

After the copy:

- Groups from the source assignment are created on the target (or reused if a group with the same name already exists).
...
- Empty target groups that have no submission history are deleted; groups with submission history are preserved so graded work stays intact.
- If the target assignment has been released and has a handout repo, new group repositories are provisioned automatically.
