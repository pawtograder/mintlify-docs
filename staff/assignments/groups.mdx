...
To copy group membership from a previous assignment, click **Copy Groups from Prior Assignment** at the top of the Groups page. This opens a dialog where you can select any earlier group assignment that already has groups formed.

<Warning>
This action runs immediately — it does not wait for you to click **Publish**. It overwrites the current group assignments for this assignment with the groups and memberships from the selected source assignment. Existing groups that become empty are removed (unless they have submission history, in which case they are preserved).
</Warning>

The dialog shows only assignments that:
- Are configured as group assignments.
- Have at least one group with members.
- Have a due date earlier than this assignment's due date.

After copying, GitHub repository permissions are automatically synced so all copied members have write access to their group repositories.
