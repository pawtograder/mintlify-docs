...
Clicking a submission link on the Test Assignment page opens the submission in **student view** — the same read-only mode used when an instructor views a student's submission. This lets you verify exactly what students will see, including:

- Which autograder tests and output are visible vs. hidden
- Whether rubric comments and scores are visible (based on release status)
- The student-facing grade display

A banner at the top of the page identifies that you are in student view and shows your name. Click **Exit student view** in the banner to return to the full instructor or grader view of the same submission — the page stays on the same submission so you can compare both perspectives side by side.

Both instructors and graders can use this preview. Graders can preview their own test submissions; only instructors can view other students' submissions in student view.

## Grading Test Submissions

After exiting student view, the submission page shows the full instructor/grader interface. You can navigate between rubrics (if applicable) from the dropdown in the right sidebar. You can also navigate between submissions (from students or even from other staff members) from the dropdown in the top bar.
