...
Clicking a submission link on the Test Assignment page opens the submission in **student view** — the same read-only experience students see, including:

- Grade-release rules (unreleased scores and comments are hidden)
- Rubric visibility settings (checks marked "never visible" are hidden)
- Hidden autograder output (staff-only test results are not shown)
- The student-view banner at the top of the page

This lets you verify exactly what students will see before releasing grades. The banner includes an **Exit student view** button that returns you to the full instructor or grader view of the same submission, so you can compare both perspectives side-by-side.

Both instructors and graders can use this preview on their own test submissions.

## Grading Test Submissions

After exiting student view, the test submission page shows the full grading interface. You can navigate between rubrics (if applicable) from the dropdown in the right sidebar. You can also navigate between submissions (from students or even from other staff members) from the dropdown in the top bar.
