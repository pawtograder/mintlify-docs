...
Clicking a submission link on the Test Assignment page opens the submission in **student view** — the same read-only mode used by the [View as student](/staff/assignments/grading-assignments#student-view) feature. This applies to both instructors and graders viewing their own test submissions.

In student view:

- A banner at the top of the page identifies whose submission you are viewing.
- Staff-only data is hidden: unreleased rubric comments, hidden autograder tests, instructor-only output tabs, and grading controls are not shown.
- Grade release rules apply: scores are only visible if the review has been released.
- The **Exit student view** button in the banner returns you to the full staff view of the same submission.

This lets you verify exactly what students will see — including which autograder output is visible, whether the rubric is shown, and how the grade is displayed — before releasing grades.
