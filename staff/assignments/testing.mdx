...
For group-only assignments, the test repository creation process properly handles group configurations, creating the appropriate group repository structure to verify that group permissions and workflows are configured correctly.
...
## Previewing submissions as a student

Clicking a submission link on the Test Assignment page opens the submission in **student view** — the same read-only mode used when an instructor views a student's submission as that student. A banner at the top of the page identifies whose view you are in.

In student view you see exactly what the student sees:

- Only released autograder tests and output tabs are shown; hidden staff-only tests and output are filtered out.
- Unreleased rubric comments and scores are hidden.
- Staff-only UI controls (grading sidebar, "Complete Review" button, "Student's Due Date" panel, etc.) are not shown.

Both instructors and graders can enter student view for their own test submissions.

**To exit student view**, click **Exit student view** in the banner. The page returns to the same submission in the full staff view, so you can compare what students see against the instructor or grader view side-by-side.

## Grading Test Submissions

After exiting student view, the submission page shows the full staff interface. You can navigate between rubrics (if applicable) from the dropdown in the right sidebar. You can also navigate between submissions (from students or even from other staff members) from the dropdown in the top bar.
