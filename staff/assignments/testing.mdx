...
Clicking a submission link opens it in **student view** — the same read-only experience students see, including:

- The "Viewing as student" banner at the top of the page.
- Grade-release rules (unreleased scores and hidden autograder output are hidden).
- Rubric visibility settings (staff-only checks are not shown).
- Hidden autograder tests and output tabs are filtered out.

This lets you verify exactly what a student will see before releasing grades. Click **Exit student view** in the banner to return to the full instructor or grader view of the same submission.

Both instructors and graders can use this student preview for their own test submissions.
