...
- **GUI view** — a visual tree editor with drag-and-drop reordering, inline forms for every field, and quick-add criterion templates. Changes in the GUI are reflected in the YAML source immediately.
- **YAML source view** — the Monaco-based text editor for power users who prefer to write or paste YAML directly. The preview pane on the right updates as you type.

Both views are bound to the same in-memory rubric, so switching between them is lossless. The editor remembers your last-used view per session.

### GUI editor

In GUI view you can:

- **Drag and drop** parts, criteria, checks, and options to reorder them. Drag handles appear on the left of each row.
- **Add criteria from templates** using the **Add criterion** menu inside each part. Templates include common patterns such as Met / Partial / Not met, multi-option scoring, and code-quality deductions.
- **Edit all fields inline** — names, descriptions, points, scoring mode, visibility, annotation targets, and more — without switching to YAML.
- **Manage check references** directly in the check row (see [Check References](#check-references-cross-rubric-context) below).

### Keyboard shortcuts (YAML source view)
...
**In the GUI editor:** Expand a check row, open the **References** section, and click **Add reference**. A dropdown lists all checks from other rubrics on the same assignment, grouped by review round. Checks in tabs with unsaved changes are shown as disabled — save those tabs first.
...
The name-keyed form resolves at save time against the sibling rubrics on the same assignment. If the target check's name is ambiguous, use `id: <check_id>` instead.

1. Navigate to the rubric preview/editor for an assignment.
2. For the check you want to augment, add a reference using the GUI or YAML.
3. Save. The relationship is stored so that, when grading, the referenced feedback appears under the referencing check.
