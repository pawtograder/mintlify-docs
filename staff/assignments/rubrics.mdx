...
- Checks can reference checks from other rubrics to surface related feedback when grading. References are configured in the rubric editor (GUI or YAML) and are cross-round only.
...
The rubric editor offers two views you can switch between at any time using the **GUI** and **YAML source** toggle buttons at the top of the editor.

### GUI editor

The GUI editor provides a visual tree of your rubric's parts, criteria, and checks. You can:

- **Drag and drop** parts, criteria, and checks to reorder them within their parent container.
- **Add criteria from templates** — click **Add criterion** inside a part to choose from built-in templates such as Met / partial / not met, Four-tier, Multi-option check, and Code-quality deductions.
- **Edit inline** — click any field to edit it directly. Changes are debounced so typing doesn't cause lag on large rubrics.
- **Manage check references** — expand the **References** section on any check to link it to a check from another rubric round (e.g., link a grading-review check to the matching self-review check).

The preview pane on the right updates in real time as you make changes in the GUI editor.

### YAML source editor

The YAML source editor gives you direct access to the underlying YAML representation. It includes:

- Syntax highlighting and schema validation via Monaco editor.
- Inline error markers for invalid YAML or schema violations.
- Points-adjustment warnings when negative point values are automatically corrected.

Switching from YAML to GUI requires the YAML to be valid. If there are errors, fix them first before toggling.

### Switching between views

Your view preference (GUI or YAML source) is remembered per browser session. Both views are bound to the same in-memory rubric, so toggling is lossless — edits made in one view are reflected in the other.

When you save, the editor uses whichever view is currently active. If you are in GUI mode, any pending debounced edits are flushed before saving.

### Unsaved changes protection

Pawtograder protects against accidental data loss. If you attempt to leave the rubric editor with unsaved changes, a confirmation dialog appears warning you that your changes will be lost. You can choose to stay and save, or discard the changes and leave.
...
3. Click **Add reference** and select a check from another rubric round (grouped by review round in the dropdown). Checks in tabs with unsaved changes are shown as disabled — save those tabs first.
4. Save. The relationship is stored so that, when grading, the referenced feedback appears under the referencing check.

**In the YAML source editor**, add a `references` list to the check:
...
References are resolved by name at save time. Same-round references are rejected. If the target check lives in a rubric tab with unsaved changes, save that tab first.
