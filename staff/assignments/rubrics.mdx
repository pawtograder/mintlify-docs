...
Set `hide_unless_assigned: true` on a rubric to prevent students from seeing its parts, criteria, and checks until a review has been assigned to them and the review's release date has passed.
  ...
  - name: Reflection
    criteria:
      - name: Effort
        ...
        is_additive: true
        checks:
          - name: Demonstrated effort
            is_annotation: false
            points: 5
```

This is useful for self-review rubrics where seeing the questions before submitting would bias the student's work. Instructors and graders always see the rubric regardless of this setting.

When combined with the **Release self-evaluation at** field on the assignment form, you can schedule the exact wall-clock time when the self-review rubric becomes visible to students — independent of each student's individual due date.
...
- Use `hide_unless_assigned: true` on self-review rubrics to prevent students from seeing the questions before they submit.
