...
Each criteria uses one of three scoring modes. The rubric editor names them "Award per check", "Deduct from total", and "Penalty only"; in YAML they are two booleans. `is_deduction_only: true` wins outright — `is_additive` is consulted only when `is_deduction_only` is `false` or omitted.

| Mode in the editor | YAML | Criteria score |
|---|---|---|
| Award per check | `is_additive: true` | sum(applied check points), capped at `total_points` |
| Deduct from total | `is_additive: false` | `total_points - sum(applied deduction points)`, floored at 0 |
| Penalty only | `is_deduction_only: true` | `-sum(applied check points)`, with the penalty capped at `total_points` — starts at zero and only ever subtracts |
