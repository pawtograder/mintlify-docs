...
In addition to the per-check settings above, rubrics have two top-level options:

- **`cap_score_to_assignment_points`**: When true, the final score is capped to the assignment's maximum points even if the rubric total exceeds it.
- **`hide_unless_assigned`**: When true, students cannot see the rubric's contents (parts, criteria, checks) until a review on it has been assigned to them and the review's release date has passed. Instructors and graders always see rubrics unconditionally. Use this for self-review rubrics where seeing the questions before submitting would bias the student's work. Pairs with the **Release self-review at** setting on the assignment.
...
```yaml
name: Sample Rubric
# hide_unless_assigned: true  # Uncomment to hide from students until a review is assigned
parts:
  - name: Code Quality
    criteria:
...
- Optional: `description`, `cap_score_to_assignment_points` (default: `false`), `hide_unless_assigned` (default: `false`)
