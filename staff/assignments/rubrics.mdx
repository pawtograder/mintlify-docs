...
When you enable **Hide from students until assigned** on a rubric, students cannot see the rubric's parts, criteria, or checks until a review has been assigned to them and the review's release date has passed. This is useful for self-review rubrics where seeing the questions before submitting would bias the student's work.

To enable this option, open the rubric editor and toggle **Hide from students until assigned** in the rubric header settings. You can also set it in YAML:
...
When `hide_unless_assigned` is `true`, the rubric is invisible to students until a `review_assignments` row exists for them on that rubric and the review's release date has passed (or is `null`). Instructors and graders always see the rubric regardless of this setting.
