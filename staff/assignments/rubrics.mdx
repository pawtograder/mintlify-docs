...
- **Rubric-level options**:
  - `hide_unless_assigned` (boolean, default `false`): When `true`, students cannot see the rubric's contents (parts, criteria, checks) until a review on it has been assigned to them. Use this for self-review rubrics where seeing the questions before submitting would bias the student's work. Instructors and graders always see rubrics regardless of this setting.
  - `cap_score_to_assignment_points` (boolean): Cap the combined score at the assignment's Points Possible.
...
Use `hide_unless_assigned: true` on a self-review rubric to prevent students from seeing the rubric questions before they submit. Once a self-review is assigned to a student, the rubric becomes visible to them.
  ...
  - name: Reflection
    criteria:
      - name: Code quality reflection
        ...
        is_additive: true
        checks:
          - name: Identified areas for improvement
            is_annotation: false
            points: 5
```
