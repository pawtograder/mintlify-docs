...
By default, students can see a rubric's structure (parts, criteria, and checks) as soon as the assignment is released. For self-review rubrics, this can bias student work — they may write their submission to match the rubric questions rather than engaging authentically.

Toggle **Hide from students until assigned** in the rubric header to gate visibility. When enabled:

- Students cannot see the rubric's contents until a review has been assigned to them **and** the review's release date has passed.
- Instructors and graders always see the rubric unconditionally.
- The setting is reflected in the YAML as `hide_unless_assigned: true`.
  ...
  - name: Reflection
    criteria:
      - name: What did you learn?
        ...
        is_additive: true
        checks:
          - name: Substantive reflection
            is_annotation: false
            points: 5
```

This option pairs with the **Release self-evaluation at** field on the assignment configuration, which lets you set an explicit wall-clock time for when self-reviews are assigned and become visible — independent of each student's individual due date.
