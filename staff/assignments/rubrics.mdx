...
Enable **Hide from students until assigned** on a rubric to prevent students from seeing its parts, criteria, and checks until a review has been assigned to them and the review's release date has passed.

Use this for self-review rubrics where seeing the questions before submitting would bias the student's work. Instructors and graders always see rubrics unconditionally.

In YAML, set `hide_unless_assigned: true` at the rubric level:
...
parts:
  - name: Reflection
    criteria:
      - name: Code quality reflection
        total_points: 5
        is_additive: true
        checks:
          - name: Identified strengths
            is_annotation: false
            points: 5
```
...
By default, self-review assignments are created for each student when their individual due date passes. You can instead set an explicit **Release self-review at** time on the assignment's self-review settings. When set:

- All students receive their self-review assignment at the same wall-clock time, regardless of per-student due-date exceptions.
- The review due date is computed as `release_at + deadline_offset hours`.
- Pairs naturally with `hide_unless_assigned` to keep the rubric hidden until the class-wide release moment.
