...
The rubric editor offers two modes you can toggle between at any time:

- **GUI mode** (default): A visual tree editor with drag-and-drop reordering, quick-add templates, and inline validation. Changes are reflected in the preview pane instantly.
- **YAML source mode**: The Monaco-based YAML editor for power users who prefer to write rubrics directly.

Your chosen mode is remembered per browser session. Switching from YAML to GUI requires valid YAML; switching from GUI to YAML flushes any pending edits first.

### GUI editor

The GUI editor organizes the rubric as a collapsible tree of parts → criteria → checks. Each level has:

- **Expand/collapse** toggle to focus on the section you're editing
- **Drag handle** (grip icon) to reorder items within their parent
- **Delete** button to remove an item
- **Add** button to insert a new item below

#### Quick-add criterion templates

Click **Add criterion** inside any part to choose from pre-built templates:

| Template | Description |
|---|---|
| Blank checkbox criterion | Single checkbox check worth 1 point |
| Met / not met (binary) | Two radio-button checks (Met / Not met) |
| Met / partial / not met | Three radio-button checks with tiered points |
| Four-tier (excellent / good / fair / poor) | Four radio-button checks |
...
Grab the grip icon (⠿) to the left of any part, criterion, check, or option and drag it to a new position. Dragging is restricted to vertical movement within the same parent container.
...
3. Click **Add reference** and select a check from another review round. Checks in rounds with unsaved changes are grayed out — save those tabs first.
4. Click **Add**. The reference is saved when you save the rubric.

**In the YAML source editor:**

Add a `references` list to a check using the name-keyed form:
...
References are resolved at save time against the other rubrics on the same assignment. Same-round references are rejected. If the target check lives in a tab with unsaved changes, the save proceeds for everything else and a toast tells you which tab to save first.
