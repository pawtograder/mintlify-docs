...
When `hide_unless_assigned: true` is set on a rubric, students cannot see the rubric's parts, criteria, or checks until a review has been assigned to them **and** the review's release date has passed (or is null).

Use this for self-review rubrics where seeing the questions before submitting would bias the student's work. Instructors and graders always see rubrics regardless of this setting.

Toggle the option in the rubric editor using the **Hide from students until assigned** switch in the rubric header form, or set it in YAML:
...
When combined with the **Release self-evaluation at** field on the assignment (see [Extensions](/staff/assignments/extensions#self-review-deadline-shift)), the rubric remains hidden until the configured release time even for students who finalize their submission early.
...
- Optional: `description`, `hide_unless_assigned` (default: `false`), `cap_score_to_assignment_points` (default: `false`)
