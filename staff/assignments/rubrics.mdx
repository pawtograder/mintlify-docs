...
- **Criteria**: A scoring rule block with:
  - `is_additive` and `is_deduction_only`: together these pick one of three scoring modes (see [How Points Are Computed](#how-points-are-computed)). Both keys may be present; `is_deduction_only: true` selects "Penalty only" whatever `is_additive` says, and `is_additive` is read only when `is_deduction_only` is `false` or absent. The editor writes one mode at a time.
...
Both booleans are optional in YAML and default to `false`, which yields "Deduct from total". If you want "Award per check" you must say `is_additive: true` explicitly.
