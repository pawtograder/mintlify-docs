...
- **Criteria**: A scoring rule block with:
  - `is_additive` and `is_deduction_only`: together these pick one of three scoring modes (see [How Points Are Computed](#how-points-are-computed)). They are mutually exclusive — `is_deduction_only` wins when set, and the editor writes only one mode at a time.
...
Each criteria uses one of three scoring modes. The rubric editor names them "Award per check", "Deduct from total", and "Penalty only"; in YAML they are two booleans, and `is_deduction_only` takes precedence over `is_additive` when both are set.
