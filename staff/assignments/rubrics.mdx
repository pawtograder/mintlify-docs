...
### Hiding Rubrics Until Assigned

You can prevent students from seeing a rubric's contents (parts, criteria, checks) until a review has been assigned to them. This is useful for self-review rubrics where seeing the questions in advance would bias the student's work.

Enable **Hide from students until assigned** in the rubric header form (GUI mode) or set `hide_unless_assigned: true` in YAML. Once enabled, students cannot read the rubric until their review assignment is created and released.

### References Across Rubrics

- Checks can reference checks from other rubrics to surface related feedback when grading. References are configured in the **References** section of each check in GUI mode, or via a `references:` list in YAML.
...
- **GUI mode** — A visual tree editor with drag-and-drop reordering of parts, criteria, and checks. Use the **Add part** and **Add criterion** menus to insert items from built-in templates (met/not met, multi-option, deduction-only, etc.).
- **YAML source mode** — A Monaco-based text editor for direct YAML editing with schema validation and autocomplete.

Both modes share the same in-memory rubric, so toggling between them is lossless. Your preferred mode is remembered per browser session.

### Drag-and-drop reordering

In GUI mode, every part, criterion, check, and option has a drag handle (⠿) on its left side. Drag items vertically within their parent to reorder them. Ordinals are updated automatically on save.

### Quick-add criterion templates

When adding a criterion in GUI mode, choose from built-in templates:

| Template | Description |
|----------|-------------|
| Blank checkbox criterion | A single checkbox check |
| Met / not met (binary) | Two radio-button checks (min/max = 1) |
| Met / partial / not met | Three radio-button checks (min/max = 1) |
| Four-tier | Excellent / good / fair / poor radio buttons |
| Multi-option check | A single check with selectable point tiers |
| Code-quality deductions | Multiple annotation checks, deduction-only |
| Penalty-only annotation | Single annotation, penalty-only |
...
- Optional: `description`, `hide_unless_assigned` (boolean, default `false`), `cap_score_to_assignment_points` (boolean)
...
**GUI mode:** Open the **References** section inside any check row. Click **Add reference**, then select a target check from another review round. Checks in unsaved sibling tabs are shown as disabled — save that tab first.

**YAML mode:** Add a `references:` list to a check. Each entry can be name-keyed (preferred) or use a numeric `id` fallback:
