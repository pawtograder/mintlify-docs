...
Two top-level rubric settings control when students can see the rubric:

- **`is_private`** — when `true`, the rubric is never visible to students (staff-only). Defaults to `false`.
- **`hide_unless_assigned`** — when `true`, students cannot see the rubric's contents (parts, criteria, checks) until a review on that rubric has been assigned to them **and** the review's release date has passed. Once assigned, the rubric becomes visible. Defaults to `false`.

`hide_unless_assigned` is designed for self-review rubrics where seeing the questions before submitting would bias the student's work. Pair it with the **Release self-review at** setting on the assignment (see [Assignment Configuration](/staff/assignments/configuration)) to control exactly when the self-review rubric becomes visible.
