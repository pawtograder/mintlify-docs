...
- Checks can reference checks from other rubrics to surface related feedback when grading. References can be managed in the GUI editor's **References** panel on each check, or declared directly in YAML (see [Check References (Cross-Rubric Context)](#check-references-cross-rubric-context)).
...
The rubric editor offers two views of the same rubric. Toggle between them with the **GUI** / **YAML source** buttons; both views are bound to the same in-memory rubric, so switching is lossless and your view preference is remembered for the session.

- **GUI editor** — a visual tree of parts, criteria, checks, and options. Drag-and-drop any item to reorder it at its level, edit every field inline, and pick a check type (checkbox / multi-option / annotation). The **Add criterion** menu offers quick-start templates: Met/Not met, Met/Partial/Not met, four-tier, multi-option, code-quality (annotation) deductions, and penalty-only. A live preview pane shows how the rubric will render in the hand-grader.
- **YAML source editor** — edit the rubric directly as YAML using the schema described below. Useful for bulk edits, copy/paste between rubrics, and version-controlled rubric files exported via the CLI.

Rubric saves are atomic: all parts, criteria, checks, and references are reconciled in a single transaction, so a rubric is never left in a partially-saved state.
...
- Optional: `id`, `description`, `file`, `artifact`, `annotation_target` (`file` | `artifact`; default behavior in UI is `file` when omitted for annotations), `max_annotations`, `data` (see below), `student_visibility` (default: `always`), `references` (name-keyed cross-rubric check references; see [Check References](#check-references-cross-rubric-context))
...
**In the GUI editor:** open the **References** panel on the check you want to augment, then search/select a check from another rubric (the current rubric's checks are excluded). Save — the relationship is stored so that, when grading, the referenced feedback appears under the referencing check as "Related Feedback from Other Reviews."

**In YAML:** add a `references:` list to a check. Each entry uses a name-keyed path identifying the referenced check by `review_round`, `part`, `criterion`, and `check` name (with a numeric `id` accepted as a fallback). References are resolved at save time and are preserved through CLI export/import, so they round-trip with your rubric YAML files.
...
References point at checks in *other* review rounds (e.g. surfacing self-review evidence during grading). They are informational only and do not contribute points.
