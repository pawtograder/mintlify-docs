...
By default, rubrics are visible to all graders as soon as they are created. You can configure a rubric to be hidden from graders until it is explicitly assigned to them. This is useful when you want to prevent graders from seeing rubric criteria before they are assigned to grade specific submissions.

To enable this, set `hide_unless_assigned: true` on the rubric. Graders will only see the rubric once they have a grading assignment that includes it.
