...
Go to the submission, then open the **Grade** tab (or the **Files** tab where rubric checks are listed alongside your code or artifacts).
</Step>
...
## When a Grade Is Changed After You Request a Regrade

If a grader re-grades your submission and removes or changes the rubric check your request was about, your request is automatically resolved with the status **Resolved** and a note that the grade was updated. You'll receive a notification.

If you still disagree after seeing the updated grade, you can **escalate** the auto-resolved request to an instructor — the escalation path remains open.

## Tracking Your Requests

To see all your regrade requests in a course, open **My Regrade Requests** from the course menu. Requests are grouped by submission. Active (non-closed) requests are listed first within each group, and groups with open requests appear at the top.

You can also see each request inline on the original submission, attached to the rubric check it's about. Requests whose backing grade was removed (because the grader re-graded) appear in a dedicated panel on the submission page with a note explaining what happened.
