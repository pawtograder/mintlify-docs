...
Go to the submission, then open the **Grade** tab (shown by default once grades are released) or the **Files** view where rubric checks are listed alongside your code or artifacts.
</Step>
...
## When a grade is changed during re-grading

If a grader deletes or changes the grade that your regrade request was about (for example, during a re-grade run), your open request is automatically resolved with the reason **"grade changed"**. You'll receive a notification explaining what happened.

This auto-resolution is **not** final — you can still escalate the request to an instructor if you disagree with the updated grade.
...
You can also see each request inline on the original submission — either attached to the rubric check it's about (for applied checks) or in the **Regrade Requests** panel on the **Grade** tab (for unapplied checks and any requests whose backing grade was later removed).
