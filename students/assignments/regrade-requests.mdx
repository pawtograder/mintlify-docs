...
When your grade has been released, your submission opens on the **Grade** tab by default. This tab shows:

- A headline score with a progress bar and a breakdown of autograder, hand-grading, and adjustment contributions.
- The **Autograder** section with individual test results (failing tests start expanded; passing tests are collapsed).
- The **Hand grading** section listing every rubric criterion with its earned/max score and the checks that were applied. Checks that were **not applied** are also shown so you can see the full rubric and request a regrade if you believe one should have been applied.
- Any manual grade adjustments (tweaks) and their notes.
- All your regrade requests for this submission, grouped together.

## Opening a Regrade Request

<Steps>
<Step title="Open the graded submission">
Go to the submission. If the grade has been released, you land on the **Grade** tab automatically. You can also open the **Files** tab to see rubric checks alongside your code.
</Step>
<Step title="Find the rubric check">
Locate the specific rubric check you want to dispute. Each check has either a **Request regrade** button (if the check was applied and you want it reconsidered) or a **Request regrade** button on an unapplied check row (if you believe a check should have been applied to your submission but wasn't).
</Step>
...
If a grader deletes or re-grades a comment that your regrade request was attached to (for example, after re-running the autograder or correcting a grading mistake), your open request is automatically resolved with the reason **"grade updated"**. You'll receive a notification.

An auto-resolved request is **not** final — you can still escalate it to an instructor if you disagree with the updated grade. Look for the **Action needed** badge on the request in your submission's Grade tab or in **My Regrade Requests**.
