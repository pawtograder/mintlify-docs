...
If a grader deletes or re-grades the comment that your regrade request was based on (for example, during a re-grading pass), your request is automatically resolved with the status **Resolved** and a note that the underlying grade was changed. You'll receive a notification.

An auto-resolved request is **not** final — you can still escalate it to an instructor if you disagree with the updated grade.

## Tracking Your Requests

To see all your regrade requests in a course, open **My Regrade Requests** from the course menu. Requests are grouped by submission, with active (non-closed) submissions listed first. Each group shows the assignment, submission number, and an "Action needed" badge if any request requires your attention (for example, an auto-resolved request you may want to escalate).

You can also see each request inline on the original submission, attached to the rubric check it's about. Requests whose underlying grade was removed appear in the **Regrade Requests** panel on the submission page with a warning and a link to escalate.
