...
You can request a regrade in two situations:

- **Applied check** — A rubric check was applied to your submission and you believe it was applied incorrectly.
- **Unapplied check** — A rubric check was *not* applied to your submission and you believe it should have been.

Both types of requests go through the same workflow: you explain your reasoning, the grader responds, and you can escalate to an instructor if needed.

<Steps>
<Step title="Open the graded submission">
Go to the submission, then open the **Grade** tab (for released grades) or the **Files** view where rubric checks are listed alongside your code or artifacts.
</Step>
...
If a grader deletes or re-grades a comment that your regrade request was based on (for example, during a re-grading pass), your request is automatically resolved with the status **Resolved** and a note that the backing grade was changed.

You will receive a notification when this happens. You can still **escalate** an auto-resolved request to an instructor if you disagree with the updated grade — auto-resolution is not final.

## Tracking Your Requests

To see all your regrade requests in a course, open **My Regrade Requests** from the course menu. Requests are grouped by submission, with active requests shown first. Each group shows the assignment, submission number, and how many requests need your attention.

You can also see each request inline on the original submission's **Grade** tab or in the rubric sidebar on the **Files** tab.
