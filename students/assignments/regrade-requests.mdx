...
Locate the specific rubric check you want to dispute. Each check has either a **Request regrade** button (if the check was applied and you want it reconsidered) or a **Request regrade — check not applied** button (if you believe a check should have been applied to your submission but wasn't). The **Grade** tab shows both applied and unapplied checks, making it easy to spot checks you think should have been applied.
</Step>
...
If a grader deletes or re-grades the comment that your regrade request was based on (for example, during a re-grading pass), your request is automatically resolved with the reason "grade updated." You'll receive a notification explaining what happened.
