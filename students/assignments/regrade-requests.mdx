...
If a grader deletes or changes the rubric comment that your regrade request was based on (for example, during a re-grade), your request is automatically resolved with the status **Resolved** and a note that the grade it referenced was changed. You'll receive a notification.

This is not a final decision — you can still **escalate** the auto-resolved request to an instructor if you disagree with the updated grade. The full comment history is preserved so the instructor has context.
