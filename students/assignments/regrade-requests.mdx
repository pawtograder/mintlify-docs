...
Go to the submission, then open the **Grade** tab or the **Files** view where rubric checks are listed alongside your code or artifacts.
</Step>
...
If a grader re-grades your submission and removes or changes the rubric check that your regrade request was about, your request is automatically resolved with the reason **"grade updated"**. You'll receive a notification explaining what happened.

An auto-resolved request is **not** final — you can still escalate it to an instructor if you disagree with the updated grade. Look for the **"Action needed"** badge on the request in your regrade requests list.

## Tracking Your Requests

To see all your regrade requests in a course, open **My Regrade Requests** from the course menu. Requests are grouped by submission, with active (non-closed) requests shown first. Each group shows the assignment, submission number, and an **"Action needed"** badge if any request requires your attention.
